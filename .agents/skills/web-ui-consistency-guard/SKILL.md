...
4. 如果发现重复 JSX、handler 或列表逻辑，检查现有组件、hooks、utils 和页面模式，确认复用价值后按 Web 目录约定收敛。
...
- 在发现重复 UI 结构或重复 handler 时，检查既有实现并按 Web 目录约定收敛。
...
- [ ] 是否检查组件复用；发现重复时是否按 Web 目录约定完成收敛或说明不提取的理由。
