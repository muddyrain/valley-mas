import type { LunarDateInfo } from './types';
import { formatDateKey, parseDateKey } from './utils';

const LUNAR_INFO = [
  0x04bd8, 0x04ae0, 0x0a570, 0x054d5, 0x0d260, 0x0d950, 0x16554, 0x056a0, 0x09ad0, 0x055d2, 0x04ae0,
  0x0a5b6, 0x0a4d0, 0x0d250, 0x1d255, 0x0b540, 0x0d6a0, 0x0ada2, 0x095b0, 0x14977, 0x04970, 0x0a4b0,
  0x0b4b5, 0x06a50, 0x06d40, 0x1ab54, 0x02b60, 0x09570, 0x052f2, 0x04970, 0x06566, 0x0d4a0, 0x0ea50,
  0x06e95, 0x05ad0, 0x02b60, 0x186e3, 0x092e0, 0x1c8d7, 0x0c950, 0x0d4a0, 0x1d8a6, 0x0b550, 0x056a0,
  0x1a5b4, 0x025d0, 0x092d0, 0x0d2b2, 0x0a950, 0x0b557, 0x06ca0, 0x0b550, 0x15355, 0x04da0, 0x0a5d0,
  0x14573, 0x052d0, 0x0a9a8, 0x0e950, 0x06aa0, 0x0aea6, 0x0ab50, 0x04b60, 0x0aae4, 0x0a570, 0x05260,
  0x0f263, 0x0d950, 0x05b57, 0x056a0, 0x096d0, 0x04dd5, 0x04ad0, 0x0a4d0, 0x0d4d4, 0x0d250, 0x0d558,
  0x0b540, 0x0b6a0, 0x195a6, 0x095b0, 0x049b0, 0x0a974, 0x0a4b0, 0x0b27a, 0x06a50, 0x06d40, 0x0af46,
  0x0ab60, 0x09570, 0x04af5, 0x04970, 0x064b0, 0x074a3, 0x0ea50, 0x06b58, 0x05ac0, 0x0ab60, 0x096d5,
  0x092e0, 0x0c960, 0x0d954, 0x0d4a0, 0x0da50, 0x07552, 0x056a0, 0x0abb7, 0x025d0, 0x092d0, 0x0cab5,
  0x0a950, 0x0b4a0, 0x0baa4, 0x0ad50, 0x055d9, 0x04ba0, 0x0a5b0, 0x15176, 0x052b0, 0x0a930, 0x07954,
  0x06aa0, 0x0ad50, 0x05b52, 0x04b60, 0x0a6e6, 0x0a4e0, 0x0d260, 0x0ea65, 0x0d530, 0x05aa0, 0x076a3,
  0x096d0, 0x04bd7, 0x04ad0, 0x0a4d0, 0x1d0b6, 0x0d250, 0x0d520, 0x0dd45, 0x0b5a0, 0x056d0, 0x055b2,
  0x049b0, 0x0a577, 0x0a4b0, 0x0aa50, 0x1b255, 0x06d20, 0x0ada0, 0x14b63, 0x09370, 0x049f8, 0x04970,
  0x064b0, 0x168a6, 0x0ea50, 0x06b20, 0x1a6c4, 0x0aae0, 0x092e0, 0x0d2e3, 0x0c960, 0x0d557, 0x0d4a0,
  0x0da50, 0x05d55, 0x056a0, 0x0a6d0, 0x055d4, 0x052d0, 0x0a9b8, 0x0a950, 0x0b4a0, 0x0b6a6, 0x0ad50,
  0x055a0, 0x0aba4, 0x0a5b0, 0x052b0, 0x0b273, 0x06930, 0x07337, 0x06aa0, 0x0ad50, 0x14b55, 0x04b60,
  0x0a570, 0x054e4, 0x0d160, 0x0e968, 0x0d520, 0x0daa0, 0x16aa6, 0x056d0, 0x04ae0, 0x0a9d4, 0x0a2d0,
  0x0d150, 0x0f252, 0x0d520,
];

const LUNAR_MONTH_NAMES = [
  '正月',
  '二月',
  '三月',
  '四月',
  '五月',
  '六月',
  '七月',
  '八月',
  '九月',
  '十月',
  '冬月',
  '腊月',
];

const LUNAR_DAY_PREFIX = ['初', '十', '廿', '三'];
const LUNAR_DAY_DIGITS = ['日', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十'];
const MIN_LUNAR_YEAR = 1900;
const MAX_LUNAR_YEAR = 2100;
const BASE_DATE_UTC = Date.UTC(1900, 0, 31);
const DAY_MS = 86_400_000;

export function getLunarDateInfo(value: Date | string): LunarDateInfo | null {
  const date = value instanceof Date ? value : parseDateKey(value);
  if (!date || Number.isNaN(date.getTime())) return null;

  const dateKey = formatDateKey(date);
  const [year, month, day] = dateKey.split('-').map(Number);
  if (year < MIN_LUNAR_YEAR || year > MAX_LUNAR_YEAR) return null;

  let offset = Math.floor((Date.UTC(year, month - 1, day) - BASE_DATE_UTC) / DAY_MS);
  if (offset < 0) return null;

  let lunarYear = MIN_LUNAR_YEAR;
  let yearDays = getLunarYearDays(lunarYear);
  while (lunarYear < MAX_LUNAR_YEAR && offset >= yearDays) {
    offset -= yearDays;
    lunarYear += 1;
    yearDays = getLunarYearDays(lunarYear);
  }

  const leapMonth = getLeapMonth(lunarYear);
  let lunarMonth = 1;
  let isLeapMonth = false;

  while (lunarMonth <= 12) {
    const monthDays = isLeapMonth
      ? getLeapMonthDays(lunarYear)
      : getLunarMonthDays(lunarYear, lunarMonth);

    if (offset < monthDays) break;
    offset -= monthDays;

    if (leapMonth === lunarMonth && !isLeapMonth) {
      isLeapMonth = true;
    } else {
      if (isLeapMonth) isLeapMonth = false;
      lunarMonth += 1;
    }
  }

  const lunarDay = offset + 1;
  const monthName = `${isLeapMonth ? '闰' : ''}${LUNAR_MONTH_NAMES[lunarMonth - 1]}`;
  const dayName = formatLunarDay(lunarDay);

  return {
    year: lunarYear,
    month: lunarMonth,
    day: lunarDay,
    isLeapMonth,
    monthName,
    dayName,
    text: lunarDay === 1 ? monthName : dayName,
  };
}

function getLunarYearDays(year: number): number {
  let sum = 348;
  const info = LUNAR_INFO[year - MIN_LUNAR_YEAR];
  for (let bit = 0x8000; bit > 0x8; bit >>= 1) {
    if (info & bit) sum += 1;
  }
  return sum + getLeapMonthDays(year);
}

function getLeapMonth(year: number): number {
  return LUNAR_INFO[year - MIN_LUNAR_YEAR] & 0xf;
}

function getLeapMonthDays(year: number): number {
  if (getLeapMonth(year) === 0) return 0;
  return LUNAR_INFO[year - MIN_LUNAR_YEAR] & 0x10000 ? 30 : 29;
}

function getLunarMonthDays(year: number, month: number): number {
  return LUNAR_INFO[year - MIN_LUNAR_YEAR] & (0x10000 >> month) ? 30 : 29;
}

function formatLunarDay(day: number): string {
  if (day === 10) return '初十';
  if (day === 20) return '二十';
  if (day === 30) return '三十';
  return `${LUNAR_DAY_PREFIX[Math.floor(day / 10)]}${LUNAR_DAY_DIGITS[day % 10]}`;
}
