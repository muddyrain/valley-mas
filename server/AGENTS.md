...
- Life Trace Pantry AI 拍照分析优先使用 Gemini：`GEMINI_API_KEY`、`GEMINI_API_BASE_URL`、`GEMINI_VISION_MODEL`、`LIFE_TRACE_PANTRY_PHOTO_AI_PROVIDER`、`LIFE_TRACE_PANTRY_PHOTO_AI_TIMEOUT_SECONDS`；未配置时回退 ARK 图片/文本模型。OCR 拍照分析默认使用 `server/scripts/paddle_ocr_text.py` + PaddleOCR venv，可用 `LIFE_TRACE_PANTRY_PHOTO_OCR_COMMAND` 覆盖，不回退 AI。
