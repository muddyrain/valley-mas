...
func RegisterWorkflowCapabilities(registry *Registry) error {
	...
	quality["default"] = "1K"
	variationMode := inputField("string", "画面变化", "控制相同画面描述再次运行时的变化幅度。", "balanced")
	variationMode["enum"] = []string{"precise", "balanced", "exploratory"}
	variationMode["default"] = "balanced"
	imageRecipe := inputField("string", "创作用途", "选择成图用途和构图侧重点。", "free")
	imageRecipe["enum"] = []string{"free", "cover"}
	imageRecipe["default"] = "free"
	imageSubjectContext := inputField("string", "文章内容", "绑定文章标题、摘要或正文，确定封面的核心主题。", "选择上游文章标题、摘要或正文")
	imageSubjectContext["allowFixedValue"] = true
	imagePrompt := inputField("string", "补充画面要求", "可选，补充主体、场景、风格或构图偏好。", "例如：冷色科技插画，主体位于画面左侧")
	...
	definitions := []struct {
		...
		{ToolCapability{ID: CapabilityGenerateAIImage, Name: "图片生成", Description: "使用模型目录中的图片模型生成并保存图片", Category: "image", SideEffect: "model_and_storage", ModelCost: 1, InputSchema: schemaWithFields([]string{"modelId", "aspectRatio", "quality"}, map[string]map[string]any{"recipeId": imageRecipe, "modelId": imageGenerationModel, "subjectContext": imageSubjectContext, "prompt": imagePrompt, "aspectRatio": aspectRatio, "quality": quality, "variationMode": variationMode, "referenceImage": inputField("string", "参考图片", "可选，绑定上游图片地址；所选模型必须支持参考图。", "选择图片地址变量")}), OutputSchema: fields(field("generationId", ValueTypeString), field("imageUrl", ValueTypeString), field("url", ValueTypeString), field("width", ValueTypeNumber), field("height", ValueTypeNumber), field("model", ValueTypeString), field("size", ValueTypeString)), AIUsage: "生成通用图片或文章封面时使用；文章封面应选择 cover 并绑定标题、摘要或正文作为主题上下文", UI: map[string]any{"fields": map[string]any{"subjectContext": map[string]any{"editor": "multiline", "label": "文章内容"}, "prompt": map[string]any{"editor": "multiline", "action": "prompt_library", "label": "补充画面要求"}}, "numberConfig": map[string]any{"key": "timeoutSeconds", "label": "超时设置（秒）", "description": "60 到 600 秒，默认 240 秒。", "min": 60, "max": 600, "default": 240}}}, GenerateAIImageCapabilityAdapter{}},
