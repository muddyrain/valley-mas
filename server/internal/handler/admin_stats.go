...
// GetStats 获取统计数据
// @Summary      获取系统统计数据
// @Description  返回用户数、资源数、下载数等统计信息
// @Tags         管理后台 - 数据统计
// @Accept       json
// @Produce      json
// @Security     Bearer
...
func GetStats(c *gin.Context) {
	...
	var topUserResourceCounts []struct {
		UserID        model.Int64String
		ResourceCount int64
	}
	db.Model(&model.Resource{}).
		Select("user_id, COUNT(id) as resource_count").
		Where("user_id IS NOT NULL AND user_id != '' AND user_id != '0'").
		Group("user_id").
		Order("resource_count DESC").
		Limit(5).
		Scan(&topUserResourceCounts)

	topUsers := make([]UserResourceStats, 0, len(topUserResourceCounts))
	for _, row := range topUserResourceCounts {
		var user model.User
		if err := db.First(&user, "id = ?", row.UserID).Error; err != nil {
			continue
		}
		topUsers = append(topUsers, UserResourceStats{
			UserID:        row.UserID,
