...
func Setup(cfg *config.Config) *gin.Engine {
	...
	handler.StartAIMotionStickerWorker(context.Background())
	r := gin.Default()
	...
	api := r.Group("/api/v1")
	{
		...
		auth.Use(middleware.Auth(cfg))
		{
			...
			auth.GET("/ai/motion-sticker-options", handler.ListAIMotionStickerOptions)
			auth.GET("/ai/motion-stickers", handler.ListAIMotionStickerGenerations)
			auth.POST("/ai/motion-stickers", handler.CreateAIMotionStickerGeneration)
			auth.GET("/ai/motion-stickers/:generationId", handler.GetAIMotionStickerGeneration)
			auth.GET("/ai/motion-stickers/:generationId/content", handler.GetAIMotionStickerContent)
			auth.DELETE("/ai/motion-stickers/:generationId", handler.DeleteAIMotionStickerGeneration)
			auth.POST("/ai/image-style-analyses", handler.CreateAIImageStyleAnalysis)
			...
			auth.GET("/ai/apps/:appId/artifacts/:artifactId/download-url", handler.GetAIAppArtifactDownloadURL)
			...
			auth.POST("/ai/apps/:appId/tasks/:taskId/retry", handler.RetryAIAppTask)
			auth.POST("/ai/apps/:appId/tasks/:taskId/approvals/:approvalId/decision", handler.DecideAIAppToolApproval)
			auth.POST("/ai/apps/:appId/tasks/:taskId/clarifications/:clarificationId/decision", handler.DecideAIAppTaskClarification)
			...
			auth.POST("/ai/skills/install", handler.InstallAISkill)
			auth.DELETE("/ai/skills/:skillId", handler.ArchiveAISkill)
			...
			auth.POST("/ai/knowledge-bases/:knowledgeBaseId/retrieval-tests", handler.TestAIKnowledgeRetrieval)
			...
			auth.GET("/workflows/:id/collaboration/archived-sessions/:sessionId", handler.GetArchivedWorkflowCollaborationSession)
			...
			auth.GET("/workflows/:id/collaboration/attachments/:attachmentId", handler.DownloadWorkflowCollaborationAttachment)
			...
			auth.POST("/workflows/:id/runs/:runId/nodes/:nodeId/cancel", handler.CancelWorkflowRunNode)
			auth.POST("/workflows/:id/runs/:runId/retry", handler.RetryWorkflowRun)
			auth.POST("/workflows/:id/runs/:runId/resume", handler.ResumeWorkflowRun)
