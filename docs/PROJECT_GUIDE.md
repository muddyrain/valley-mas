...
- 雨迹私有创作室与实验室：`apps/web/src/layouts/StudioLayout.tsx`、`apps/web/src/layouts/PrivateLabLayout.tsx`、`apps/web/src/pages/StudioHome`、`apps/web/src/pages/StudioArticles`、`apps/web/src/pages/StudioImageImport`、`apps/web/src/pages/StudioImageCreator` 与 `apps/web/src/pages/BlogCreate`。`/studio` 只面向登录站主，集中写文章、统一草稿、批量导入图片和 AI 图片；`/workbench` 使用独立布局承载智能体、工作流、知识库、提示词、技能与进阶生成能力，并保持为创作室的次级入口。旧收藏、关注、下载历史、通知和个人资料路由仍作存量兼容，但不出现在主导航与账户菜单。
...
- AI 图片创作：Web `/workbench/images` 是对话生图页，支持查看和切换 owner 私有历史会话；`/workbench/canvas` 页面、侧栏入口和站内搜索命令已移除。旧 `ai_canvas_documents` 数据和 `/ai/canvas-document` 接口仅作为存量兼容保留，当前 Web 不再调用；后续删除或迁移需单独确认数据策略。图片工作台与智能体会话共用 `ConversationComposer`，两端都可附加最多三张 5MB JPG/PNG/WebP 参考图、查看附件状态和图片生成阶段反馈；智能体输入框可通过 `/` 选择已绑定技能，图片工作台则通过单一风格选择器选择内置风格或 owner 私有已安装技能。图片工作台默认直接具备生图能力；智能体需在版本中显式绑定 `image.generate`，再使用其已绑定技能作为本轮视觉风格。两个入口保留独立历史实体，以维持图片会话与智能体版本/运行追踪的隔离。图片工作台支持对话描述与连续修改、输出用途、统一风格选择、从 owner 私有 AI 提示词模板填入画面描述、上传 1–9 张 JPG/PNG/WebP 图片识别共性视觉风格（单张最大 20MB，原图仅请求内使用）、五种画面比例、模型感知的目标分辨率、生成阶段反馈、owner 私有生成历史、下载和保存到资源库。输出用途只声明用途与结构约束，当前提供自由创作、壁纸、草图成图、文章封面、产品展示和角色头像；自由创作不注入隐藏用途提示。风格只控制色彩、材质、光线和渲染语言，内置日系动画、动画 IP、电影风景和手作毛毡，也将 owner 私有已安装技能通过图片风格适配器加入同一个选择器。提示词模板只作为画面描述，当前输入非空时由用户选择追加或替换，不会改变风格或输出用途。服务端 `AIImagePlanner` 统一解析 `recipeId + styleProfileId + subjectContext + brief + variationMode`：题材上下文只提供事实，用户 brief 保留明确要求；无参考图默认均衡变化，参考图默认精确遵循，也可选择大胆探索。每次变化会重新组合构图、视角、光线、配色与画面节奏，但不得覆盖参考结构、主体身份与数量、输出用途或用户明确描述；变化模式、种子和编译指令随生成记录留痕，Provider 只负责协议传输。博客封面助手复用同一计划器，以标题、摘要和正文要点作为题材上下文，不再注入固定二游角色提示词。技能内容只在服务端按 owner 校验后作为视觉风格使用；旧 `presetId/skillId/prompt` 请求及旧图片选项路由暂时保留兼容映射。输出用途的快速示例首屏与后续“换一些”都来自服务端受控候选池，每次即时轮换三条不同于当前项的画面描述，不调用外部模型。对话首条消息直接生成，后续消息优先引用本次对话最近一张成功图片，也可附加本地参考图或锁定历史图片；消息和会话按当前用户持久化到服务端，图片结果持久化到历史。用户统一选择具备 `image_generation` 的图片模型；`reference_image` 表示该模型还支持把附件或上一张生成图作为参考输入。尺寸能力按模型 ID 判断、Provider 只处理传输协议：`doubao-seedream-4-0-*` 支持 1K/2K/3K/4K，`gpt-image-2` 包括参考图编辑在内均支持 1K/2K/4K。参考图的主体数量、轮廓、姿态、取景和空间布局优先于输出用途、风格与文字；当前模型不支持参考图时保留模型选择，并明确按文字生成；草图成图必须附加或锁定参考图。图片调用由 SiliconFlow、Amux、ARK Provider Adapter 处理各自尺寸字段、JSON/multipart 端点与 URL/Base64 响应；目标尺寸是请求意图而非精确像素承诺，4K 请求返回达到该目标对应 1K 宽高基线的有效图片即按实际像素保存，只有低于最低可用尺寸的明显异常结果才失败，任务状态持久化在 `ai_image_generations`。生成中的任务可暂停：服务端会中止当前 Provider 请求并持久化为 `paused`；第三方生成请求不能从中间进度恢复，用户可从该条历史恢复参数后重新提交。用户直接提交的单张参考图限制为 5MB；引用服务端已持久化的生成结果时按 128MiB 下载上限处理，不套用上传限制。生成结果及其恢复副本允许 JPG、PNG、WebP、GIF、AVIF 和 BMP 等安全栅格格式，并按 128MiB 上限转存；SVG 因可携带可执行标记而不接纳。参考图 data URL 只在当前请求和后台生成过程中使用，不进入数据库或 AI 审计日志；使用临时附件或对话历史图时会额外保存一张扁平快照，供历史预览与再次创作恢复，快照上传失败只影响历史恢复，不阻断图片生成。生成结果必须转存 TOS 后才进入长期历史；保存到资源库会复制为独立对象，因此资源删除不会影响历史。智能体生图复用同一计划器、模型目录、Provider、存储与审计；原始参考图不持久化，会话只保存附件数量和生成记录 ID。
- AI 图片重试：系统不会自动重试失败任务。图片对话中的失败气泡提供手动重试，重试会复用原任务的模型、输出用途、风格、比例、清晰度和可恢复参考图，创建一条新的生成任务；服务商可能按新调用计费。无法恢复的临时参考图仍引导用户恢复参数后重新提交。
- AI 动态表情：Web `/workbench/gifs` 接收当前用户的一张 5MB 以内 JPG/PNG/WebP 单角色参考图和动作描述，创建 owner 私有的持久异步任务。默认“生图 GIF”使用具备 `image_generation + reference_image` 且当前图片 Adapter 支持多图输出的模型，一次请求默认生成 6 张连贯帧，再由服务端用固定 FFmpeg 参数编码为最大 320×320、无限循环的 GIF；火山 ARK 使用顺序组图协议，OpenAI-compatible 图片协议使用 `n` 多图请求。可选“视频增强”使用 `video_generation + reference_image` 模型，AMUX `amux_video` 适配 `/video/generations` 任务协议，固定请求 5 秒、1:1、720p、无音频视频，成功后转存 MP4，并通过 `ffprobe` 与 FFmpeg 派生 GIF。后台 worker 每 5 秒恢复和推进两类任务，技术超时 10 分钟，同一用户最多一个活动任务；存量任务默认按视频模式解释。历史、详情、内容读取和删除都按 owner 隔离，Web 通过鉴权内容接口预览与下载，不返回对象存储原始地址。服务层和 `motion_sticker.generate` 工具契约统一接收可选 `mode`，未传时默认 `image`，供页面、智能体和工作流复用；当前 AMUX 文档无取消端点，因此不提供会误导计费预期的取消操作。
- AI Agent 运行时：`server/internal/ai/agent`（领域中性 tool loop 抽象，未来可无痛迁 CloudWeGo eino）+ `server/internal/ai/tools`（Tool 接口与 Registry）。内部 Tool 可选声明输入输出、风险、确认策略和结果卡类型，契约保持 MCP 可适配，但当前不启动面向外部客户端的 MCP Server。智能体的对话与图片理解共用当前对话模型：上传图片时要求该模型同时具备 `vision` 能力，不支持时阻止本轮发送并引导切换模型；生成或编辑图片仍是独立的 `image.generate` 工具链路，要求绑定工具及具备 `image_generation` 的图片模型。智能体可按版本绑定 `image.convert` 和 `document.convert`，前者支持本轮 owner 附件的 WebP/JPG/PNG 互转并保留透明通道，后者把正常文本 PDF 尽力转换为保留主要文字、段落和分页的 DOCX；还可绑定 `document.export`，把明确正文导出为 Markdown、PDF 或 DOCX 文件卡。转换和导出结果按 owner 隔离并默认保留 72 小时，列表不返回底层对象地址，下载前由服务端再次校验 owner 与过期时间。`document.save` 将临时成果转为 owner 私有长期文档，`document.overwrite` 只允许覆盖当前 owner 的既有文档，`blog.publish` 通过既有博客工作流创建并发布文章；三类持久写操作即使配置为自动执行，也会按 Tool 契约和参数指纹进入当前任务的确认卡，拒绝自动发布。对字段完整的显式导出与博客发布请求，服务端会保守地直达已绑定工具，避免仅依赖模型是否正确发起 tool call；字段缺失时仍交给模型或结构化澄清流程，不猜测正文、文件名或可见范围。后台智能体缺少必要工具参数，或普通对话必须由用户补充信息、确认偏好后才能继续时，可调用内部 `clarification.ask`：任务进入 `needs_input`，页面显示建议答案、自由回答、跳过或拒绝卡片，用户答复作为同一运行的正式用户消息恢复任务；模型未调用工具而直接输出明确追问时，服务端会保守地转成同一结构化协议，页面不解析自由文本。阻塞问题被跳过或拒绝时会终止任务并解释所缺信息。工具失败按稳定错误码、用户可见说明和可重试标记持久化，可重试错误卡会从原用户消息创建新运行，不重复写入用户消息。知识库检索是独立限时的可选增强；详情页 SSE、后台任务、调试和公开调用共用同一降级模块，检索异常时继续生成，运行记录保存降级状态和稳定错误码，页面提示本轮未使用私有资料，公开调用响应返回知识增强状态。智能体详情页不展示版本历史，用户修改当前设置后既有会话下一轮也切换到最新配置快照。个性化由默认 `IDENTITY.md`、`USER.md`、`SOUL.md`、`AGENTS.md` 四份档案组成，运行时按身份、用户偏好、行为准则和智能体约定的顺序合成为系统指令。智能体详情页同时聚合会话产生的 owner 私有图片与成果文件。首批 Agent runtime 同时被 Life Trace 生活助理经 `LIFE_TRACE_ASSISTANT_USE_AGENT` 灰度使用。
