...
- AI 工作台平台基线：Web `/workbench` 将智能体与工作流分区展示；智能体编辑页支持不可变版本保存、历史版本恢复为新草稿、发布及当前草稿版本的 ARK 私有 SSE 调试、停止和最近运行摘要，也可绑定私有资料库。`/workbench/knowledge` 支持 Markdown/TXT/PDF 上传、异步切块与向量化状态、失败重试；在 PostgreSQL 安装 `pgvector` 且配置 `ARK_EMBEDDING_MODEL` 后，已索引资料会在智能体调试和已发布应用的公共 API 中检索并返回引用摘要。知识库绑定和检索配置会随应用版本保存快照，后续改绑不改变已发布版本。服务端 `/ai/apps` 提供 owner 隔离的应用、版本、发布状态、统一运行摘要、受控工具目录、API Key 及 JSON/SSE 公共调用；旧工作流会映射为 workflow 应用，仍通过 `/workflows/*` 维持图编辑和节点级运行历史兼容。工作流统一使用 Graph v2 的线性 DAG，未开放的 HTTP、代码、条件和循环节点在执行前会被拒绝；会话和检索调试入口仍待后续阶段实现，不能视为已开放能力。
