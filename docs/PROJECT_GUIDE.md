...
- Server 示例配置在 `server/.env.example`，包括 `DB_*`、`JWT_SECRET`、`SMTP_*`、`TOS_*`、`ARK_*`、`GEMINI_*`、`LIFE_TRACE_AI_*`、`MIND_ARENA_AI_*`、`QWEATHER_*`、`WEB_PUSH_*`。
...
- Life Trace Pantry AI 拍照分析优先使用 `GEMINI_API_KEY`、`GEMINI_API_BASE_URL`、`GEMINI_VISION_MODEL`，可用 `LIFE_TRACE_PANTRY_PHOTO_AI_PROVIDER` 强制切换 `auto` / `gemini` / `ark`，可用 `LIFE_TRACE_PANTRY_PHOTO_AI_TIMEOUT_SECONDS` 单独调整超时；未配置 Gemini 时回退 `ARK_VISION_MODEL`，再回退 `ARK_TEXT_MODEL`。OCR 拍照分析不回退 AI，留空 `LIFE_TRACE_PANTRY_PHOTO_OCR_COMMAND` 时服务端优先使用 `server/scripts/paddle_ocr_text.py` 和用户目录下的 PaddleOCR venv，也可用该变量覆盖为自定义 OCR 命令。
