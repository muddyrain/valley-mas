...
- AI 工作台平台基线：Web `/workbench` 将智能体与工作流分区展示，并提供 AI 创建预览入口。智能体编辑页支持不可变版本、发布、ARK 私有 SSE 调试、对话式流式提示词优化、简介/开场白/示例问题的字段级 AI 生成，以及应用级 AI/上传头像；AI 建议只替换当前本地草稿，不自动保存或改写已发布版本。AI 头像按当前编辑上下文生成 Q 版人物或动漫机器人形象。创建智能体时，用户确认的 `content.search` 与 owner 私有知识库会和初始版本快照在同一事务写入。`/workbench/knowledge` 支持 Markdown/TXT/PDF 上传、异步切块与向量化；在 PostgreSQL 安装 `pgvector` 且配置 `ARK_EMBEDDING_MODEL` 后，资料会进入调试、私有会话和公共 API 的版本化检索。服务端 `/ai/apps` 提供 owner 隔离的应用、版本、运行、工具、API Key 及 JSON/SSE 公共调用；AI App 私有会话返回消息、运行与安全工具轨迹，并消费会话固定版本的开场白和示例问题。旧工作流仍通过 `/workflows/*` 维持图编辑和节点历史；Graph v2 为最多 8 节点的线性 DAG，开放 `variable` 字符串模板赋值和 AI 草稿/失败解释，HTTP、代码、条件和循环在执行前继续拒绝。
