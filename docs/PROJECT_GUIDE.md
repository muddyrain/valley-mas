...
- 平台共享 Provider 首期使用 `SILICONFLOW_API_KEY`（默认 Base URL `https://api.siliconflow.cn/v1`）和 `AMUX_API_KEY`（默认 Base URL `https://api.amux.ai/v1`）；可用 `*_BASE_URL` 接入私有网关。模型不写入环境变量：管理员在 Admin「AI 模型目录」审核、导入并检测模型，标记文本、视觉、生图或向量能力；业务节点和交互界面直接选择已启用且能力匹配的模型。连接检测会以 1 token 的最小真实请求验证指定模型并返回耗时，因此会产生极少量 token 用量。Provider 密钥只保留在服务端环境变量，不支持用户自带 key。
