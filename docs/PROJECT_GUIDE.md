...
- AI Agent 运行时：`server/internal/ai/agent`（领域中性 tool loop 抽象，未来可无痛迁 CloudWeGo eino）+ `server/internal/ai/tools`（Tool 接口与 Registry）。WorkBench 智能体可绑定受控的内容搜索与 `image.generate`；生图工具固定使用版本选择的图片模型，输入只允许画面描述、比例、清晰度和当轮参考图，输出为已转存图片资产。首批 Agent runtime 同时被 Life Trace 生活助理经 `LIFE_TRACE_ASSISTANT_USE_AGENT` 灰度使用。
