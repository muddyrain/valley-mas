...
> 状态：首期产品代码完成，待浏览器、真实模型与真实数据库迁移人工验收
...
- [x] 保留智能体共享会话组件的既有测试，并用全量 Web 测试守住现有行为。
- [x] 为服务端补齐唯一时间线迁移、后台任务创建、owner 隔离、冲突、撤销和旧路径只读测试。
- [x] 为 `server/internal/workflow/operations.go` 覆盖节点字段、连线、删除目标、局部布局触及路径和逆操作。
...
- [x] 工作流领域 DTO 保持在 API 适配层，不扩散到共享消息与输入组件。
- [x] `ConversationComposer`、`ConversationMessageBubble` 和附件卡通过 props 与 callback 注入工作流行为。
- [x] 不修改智能体会话的排队、停止、部分回复、正式消息替换、附件和自动滚动实现；全量 Web 测试通过。
- [x] 工作流任务呈现覆盖 `queued / running / waiting_approval / succeeded / failed / cancelled / conflicted`。
...
- [x] 工作流 worker 复用统一状态名、owner 并发上限、取消和恢复语义；AI App 与工作流保留各自领域领取和持久化适配器。
- [x] 为工作流协作增加持久任务、附件、风险确认和已应用变更模型。
- [x] 同一工作流严格串行；不同工作流可并行；owner 的 AI App 与工作流任务共享 3 个运行、20 个未完成任务上限。
- [x] 服务启动时恢复可安全重跑的工作流协作任务；已进入写入事务的任务通过幂等键避免重复应用。
- [x] 任务执行使用固定工作流系统指令、完整草稿、当前能力目录和 Graph operation 契约；修改任务不做冗余的只读工具往返。
- [x] 附件复用智能体文件类型、大小、解析、owner 隔离和单轮注入规则。
- [x] 单次技能只注入本任务；绑定工作流节点必须来自用户明确意图和可审计操作。
...
验收：离开页面和服务重启后任务继续；同工作流不并发写入；阶段状态、任务终态、停止和冲突均可重放，结构化最终回复原子落库。

首期不把 AI App 与工作流 worker 强行合并为单一实现，也不向页面流式暴露模型生成中的结构化 JSON；这两项不改变本期产品能力，作为后续架构与体验优化评估。

## 阶段 4：权限、试运行、通知与唯一时间线

- [x] 新增唯一工作流协作读取、任务、取消、附件、上下文重置、风险确认决策和变更撤销接口。
- [x] 草稿修改和静态校验直接执行；AI 发起的试运行、发布和现有触发器启停创建 owner 私有确认。
- [x] 确认参数与指纹只保存在服务端，页面只读取风险摘要；批准只对当前任务的同一调用有效。
- [x] 为等待确认、成功、失败和冲突创建幂等站内通知；工作流列表返回并展示最新协作状态。
- [x] 迁移旧多会话：最近会话成为 canonical，上下文重置使用边界标记，旧会话和 pending 提案只读保留。
- [x] 保留旧 `/ai/workbench/copilot/*` 只读兼容；旧工作流会话、消息和提案不删除且不再接受写操作。
...
- [x] 在右侧“AI 协作”Tab 展示任务、未读、确认、冲突和完成状态；切换到节点信息不影响任务。
- [x] 增加工作流结果卡：变更摘要、节点定位、风险、查看画布和撤销。
- [x] AI 更新到达时在不抢焦点的前提下同步画布；整次修改映射为一个浏览器撤销项。
- [x] 节点信息增加“询问 AI”，发送时冻结节点上下文；结果卡节点反向定位节点信息。
- [x] 接入模型、单次 Skill、图片/PDF/Markdown/TXT/JSON/CSV 附件和停止按钮。
- [x] 保持桌面单右栏与移动端全高 Sheet；任务完成不强制切回 AI Tab。
...
- [x] Web 类型、改动文件静态检查、定向测试、全量测试与覆盖率门禁通过。
- [x] Server 受影响包测试、全量 `go test ./...` 和构建通过。
- [ ] 使用用户当前 Chrome 会话验证桌面与移动端关键路径、附件、后台完成、确认、冲突、撤销、通知和控制台。
- [ ] 使用可用模型至少跑通一次“附件生成工作流 → 后台直接修改 → 离开返回 → 撤销”。
- [x] 同步 `docs/PROJECT_GUIDE.md` 中多会话、提案、权限和交互描述；保留本计划到人工验收完成后归档。
- [x] 计划索引、文档链接等价扫描、改动文件编码检查和 `git diff --check` 通过。
- [ ] 完成验收后把本计划标记完成并移入 `docs/plans/archive/2026-08/`。

验证基线说明：仓库全量 Biome 仍有与本次无关的既有格式问题；全量 UTF-8 脚本受 `core.quotePath` 和 8 个既有 BOM 文件影响；Windows Bash 服务不可用，因此文档链接按脚本相同规则直接扫描。上述问题不涉及本次改动文件。
...
7. 先制造失败并连续续跑，再让 AI 修改工作流；确认运行详情保留续跑链中已完成节点，新画布不继承旧图运行覆盖。
...
- [x] 风险试运行/发布/触发器确认、单次 Skill、工作流列表状态、节点询问和未读状态已落地。
- [x] 旧会话和旧提案支持 owner 私有按需只读查看，不返回候选草稿 JSON。
- [x] 续跑链节点状态合并和 AI 改图后的旧运行覆盖失效已完成自动化测试，并在当前 Chrome 工作流页面确认旧覆盖不再残留。
- [ ] 完整浏览器、真实模型和真实 PostgreSQL/MySQL 迁移验收尚未完成；验收通过后再归档本计划。
