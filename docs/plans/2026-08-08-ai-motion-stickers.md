...
> 状态：生图 GIF 已作为默认链路落地，视频模型保留为可选增强；真实 Seedream 生图已暴露角色漂移并完成首轮补强，等待人工复验
> 日期：2026-08-09
...
- [x] 将图一明确为角色身份基准，动作中的人称代词只指向图一主体，并禁止改变物种、主体和标志性外观。
- [x] 火山 ARK 参考图组图移除未文档化的 `n` 参数，使用顺序组图参数并提高参考图提示遵循度。
- [x] 模型目录的 `reference_image` 检测从“请求成功即通过”改为校验输出是否保留未写入提示词的参考图色块结构。
- [x] 生图任务在 GIF 编码前增加本地显著角色配色漂移检查；彩色角色明显换角时以 `identity_mismatch` 失败，不保存错误作品，单色参考图保持保守放行。
- [ ] 使用同一熊猫参考图和“坐在沙发上玩手机”动作重新完成付费模型验收，确认主体不再被替换成人类，并复查动作连贯和循环效果。
