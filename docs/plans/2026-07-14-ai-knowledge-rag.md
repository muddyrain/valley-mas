...
> 状态：Markdown/TXT/PDF 核心链路、真实索引进度、单篇删除与 PostgreSQL/ARK 实库验收已完成
...
- [x] 新增知识库 CRUD、Markdown/TXT/PDF 文档上传、删除、重试和状态查询接口；限制文件大小、每文档分段数。无法提取文本的 PDF 记录为 `DOCUMENT_PARSE_FAILED`，不创建空分段或触发 embedding。
- [x] Web 新增 `/workbench/knowledge` 入口与文档列表，显示待处理、索引中、成功、失败状态与真实索引进度；支持删除单篇文档及其分段。

## 阶段 B：异步解析、切块与检索

- [x] 实现 Markdown/TXT/PDF 文本提取；PDF 仅在可用解析器成功提取时入库，否则记录 `DOCUMENT_PARSE_FAILED`。
...
- [x] 增加 owner 隔离、embedding 配置缺失、PDF 解析失败、删除清理和引用返回的服务端测试；PostgreSQL/ARK 集成已完成真实环境验收。
...
- `pnpm check:harness`
- PostgreSQL 环境已手动验证：上传 TXT/Markdown/PDF → 索引成功 → 智能体回答含引用；缺少 embedding 配置时返回 503。扫描件、加密或损坏 PDF 显示解析失败且不提供索引重试。
