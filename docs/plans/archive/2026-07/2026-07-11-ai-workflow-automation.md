...
| `server/migrations/045_add_workflow_runtime_records.sql` | 已有数据库的 workflow run 升级和节点运行记录表 |
...
| `server/internal/handler/workflow_runtime_test.go` | 鉴权、multipart、真实事件、历史隔离和错误分支集成测试 |
...
| `apps/web/src/components/workflow/runSession.ts` | 当前运行会话、runId 防串流与单节点展开状态 |
| `apps/web/src/components/workflow/NodeRunInspector.tsx` | 画布节点的安全输入/输出悬浮详情卡 |
| `apps/web/src/components/workflow/RunPanel.tsx` | 文件、标签、分组输入、运行控制与最终草稿入口 |
| `apps/web/src/pages/WorkflowEditor/index.tsx` | 正式博客模板、保存前校验、SSE 会话与运行态连线 |
...
- Create: `server/migrations/045_add_workflow_runtime_records.sql`
...
`045_add_workflow_runtime_records.sql` 必须使用幂等 DDL：为历史表补 `inputs` 与新增字段，创建 `workflow_node_runs` 和 `(workflow_run_id, sequence)`、`(workflow_id, created_at)`、`status` 索引。将新模型加入 `contentDomainMigrationModels()`，确保 `DB_AUTO_MIGRATE=true` 的本地环境也能建立表。

- [x] **Step 4: 运行模型与迁移范围测试**
...
- Create: `server/internal/handler/workflow_runtime_test.go`

- [x] **Step 1: 编写 multipart SSE 与权限隔离测试**
...
- [x] **Step 2: 把运行请求改为 FormData 并保持 SSE 解析**
...
`RunPanel` 将 `markdownFile` 渲染为单文件选择，`tagIds` 渲染为标签多选，`groupId` 和 `visibility` 使用已有数据源/选项。运行会话按 `runId + nodeId` 合并 SSE，拒绝旧会话迟到事件；成功后显示“打开草稿”按钮，链接来自 `end` 输出。每个节点通过不改变节点尺寸或连线位置的悬浮详情卡显示截断后的安全 JSON 输入/输出、毫秒耗时或可复制错误。本期不做历史节点回放。
