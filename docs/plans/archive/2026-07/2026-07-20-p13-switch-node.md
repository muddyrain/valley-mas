...
> 前置规格：[选择器（Switch）节点设计规格](../specs/2026-07-20-p13-switch-node-design.md)。代码与浏览器运行时验收已完成，已通过手动验证；验收记录以用户反馈为准。
...
3. [x] 浏览器验收：本地 Web 已完成手动验收（2026-07-21）。在可用账号下完成 `contentType` 输入、文章/资源/default 三路运行、各路径 skipped 验证，并通过保存、刷新与重试回归行为验证。
4. [x] 更新 P13.3 状态与项目指南的 Graph v4 节点清单；真实运行验收已完成，路线图状态同步更新。
