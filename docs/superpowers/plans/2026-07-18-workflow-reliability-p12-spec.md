...
P12 以 Graph v4 为唯一工作流协议，不恢复 Graph v2/v3 或旧业务节点。开始前必须完成 P11 的开发库清理 dry-run、真实 ARK 的空白生成与封面开关真假运行验收；若开发数据库或 ARK 环境不可用，先补可重复的隔离环境。
...
- 覆盖“封面开关为 false 时零生成调用、true 时仅一次调用”“失败后 AI 修复提案”“含写入节点的重试二次确认”等回归。
