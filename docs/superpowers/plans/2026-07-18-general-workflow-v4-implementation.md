...
Graph v4 代码、自动化回归和核心编辑器浏览器交互已落地。用户已在实际页面确认画布对齐辅助线可见，并能走通新工作流；既有工作流数据按当前决策保留，不作为 P11 关闭门槛。封面节点执行策略已改为节点自身控制，需按新语义补一次真实 ARK 提案与运行验收，P11 暂不关闭。
...
- 本记录据此不再将“辅助线是否可见”或“新 Graph v4 工作流是否可运行”列为待验项；既有数据保留，不进行旧工作流清理。封面节点策略已调整：默认直接生成，需要时仅在节点“生成条件”绑定已有上游布尔变量；需按此新语义补充真实 ARK 验收后关闭 P11。

## 2026-07-18 真实 ARK 封面节点验收（已调整）

- 用户在实际环境确认：空白 AI 创建工作流可完成，旧语义下的局部封面提案可生成并应用。
- 待验：新提案只插入封面节点、不新增 Start 输入；默认运行时封面节点成功生成一次。若绑定已有上游布尔变量，`false` 时节点 `skipped` 且零生成调用、`true` 时只生成一次。
