...
不包含 macOS 系统声音、暂停/恢复、GIF/FFmpeg 或 MP4 转码、视频编辑、上传、Web 唤起协议、自动更新、正式分发所需的 Developer ID 签名与公证，以及 Linux 适配。Electron 的桌面音频 loopback 当前只支持 Windows；macOS 系统声音若要落地，需要另行确认并引入原生捕获能力，当前不会伪装为可用。
...
- Windows NSIS 未签名，可能出现 SmartScreen 提示。macOS 打包会优先自动使用钥匙串中的 Apple 代码签名证书或 `CSC_NAME` 指定的身份；没有有效证书时才回退到 `com.valley.screenrecorder` 的本机 ad-hoc 签名并输出明确警告。ad-hoc 签名不受 Gatekeeper 信任，代码变化后也可能需要重新添加录屏权限；持续开发应使用 Apple Development，正式分发应使用 Developer ID Application 并完成 notarization。
