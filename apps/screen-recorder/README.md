...
- 捕获模式栏：触发区域截图或录屏后，在选择层顶部显示“截图 / 录屏”。同一次选区会话的两种模式共用捕获底图，切换不重新捕获、不隐藏窗口；跨显示器时重新获取目标屏幕。录屏设置阶段仍显示这张底图，开始录制后捕获实时屏幕。
...
- 选区交接：区域选择先获取不含浮层的显示器图像，然后立即显示可操作的选区；底图解码期间仍可拖拽。选区和编辑器共用同一个已解码图像节点，边框几何在动画帧内提交；默认移动工具直接显示底图，标注或导出时才绘制裁剪画布。预览通过仅在当前捕获期间有效的本地随机资源 URL 传输无损位图，不在交互路径同步编码 PNG；最终导出仍为 PNG。
- 长截图：从当前选区开始，用户滚动目标内容时按固定选区连续采样；采集期间蓝色选区边框和选区外遮罩保持可见，采样不会反复隐藏、恢复捕获浮层。浮层只显示实时长图预览以及取消、完成按钮，整个浮层按右侧、左侧、下侧、上侧的顺序寻找选区外空间，全部放不下时才进入选区内右下角；右侧或左侧预览默认贴齐选区底边，内容增加时向上生长，达到选区高度或屏幕可用空间上限后内部可滚动并自动跟随最新内容，横向滚动条始终关闭，纵向只在滚动时短暂显示滑块，不显示轨道底座。拼接使用有上限的粗匹配加局部精确匹配，只保留每帧新增的像素行，避免长时间采集持续积压完整帧；向下越过已捕获底部时追加新内容，向上越过首帧时在顶部前插，来回滚动不会重复写入已捕获区域。拼接会降低两侧固定区域对滚动位置的干扰；固定侧栏只保留首屏内容，后续增量区域使用稳定背景延伸，避免选中条或图标周期性重复。滚动过快导致连续重叠丢失时，浮层会提示用户回到截图中断位置并放慢速度，提示至少保持一段可感知时间，恢复匹配后可继续采集；点击完成前还会再采集一次当前可见画面。点击完成只把整张长图复制到系统剪贴板并关闭浮层，不自动写入文件或再弹出结果窗口；取消按钮、右键或全局 `Esc` 取消时不产生结果，即使焦点仍在被截图应用也能直接关闭长截图。单张最大高度 30,000 像素，动态内容、视频、固定悬浮元素或缺少可识别重叠的页面可能无法正确拼接。
...
pnpm --filter @valley/screen-recorder runtime:performance
...
`runtime:performance` 在构建产物上启动隔离的 Electron 配置，默认使用生成的屏幕图像，通过应用注册的快捷键回调和 Chromium 输入事件验证选区拖拽、连续 Tab 切换、编辑器交接、缩放、PNG 导出尺寸与像素，以及从录屏入口切回截图。它不会注册或修改已安装应用的系统快捷键，导出只写入自动清理的临时目录。脚本同时记录首帧、切换和交接耗时，并检查切换不重新捕获、不隐藏窗口、交接不出现空白帧。它验证应用回调到渲染的路径，不包含操作系统派发实体按键的延迟。

macOS 已授予开发 Electron 屏幕录制权限时，可运行 `SCREEN_RECORDER_PERF_NATIVE=1 pnpm --filter @valley/screen-recorder runtime:performance` 验证真实捕获；未授权时直接失败，不主动申请权限。测试期间不要操作测试浮层；多显示器跨屏跟随和 Windows 平台仍需分别验收。
