...
- 长截图：从当前选区开始，用户滚动目标内容时按固定选区连续采样；采集期间始终显示蓝色选区边框、选区外遮罩、累计段数和像素高度，并在控制窗中实时追加窄图预览。拼接使用有上限的粗匹配加局部精确匹配，只保留每帧新增的像素行，避免长时间采集持续积压完整帧。完成后显示整图预览、文件路径、打开图片和打开所在文件夹入口；控制条支持完成及右键/`Esc` 取消。单张最大高度 30,000 像素，动态内容、视频、固定悬浮元素或缺少可识别重叠的页面可能无法正确拼接。
...
- 主进程：`electron/main.ts` 管理窗口、选择显示器、`desktopCapturer`、托盘、全局快捷键、开机自启动、捕获授权、原生目录/另存为对话框、保存目录和文件写入；长截图的后续帧捕获、重叠检测与拼接也留在主进程。
...
- 所有 BrowserWindow 启用上下文隔离和 sandbox、关闭 Node 集成、拒绝新窗口，并对控制窗口、选择层、录制状态提示层和悬浮控制条启用内容保护。
...
4. 再次截图并点击“保存截图文件”，确认系统另存为对话框出现、取消对话框后编辑器仍保留、选择目标路径后生成有效 PNG。点击“长截图”，确认滚动期间选区边框、选区外遮罩、段数、像素高度和控制窗内实时预览持续更新，预览自动跟随最新内容且操作没有明显冻结；完成后确认整图预览、打开图片和打开文件夹入口存在，输出高度大于原选区；另一次通过右键取消并确认不生成文件。
