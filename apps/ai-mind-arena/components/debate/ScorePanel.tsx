...
export function ScorePanel({ session, result, currentRound, scores }: ScorePanelProps) {
  ...
  return (
    <aside className="arena-panel flex min-h-0 flex-col overflow-hidden px-3 py-3 text-white xl:px-4 xl:py-4">
      ...
      <div className="thin-scrollbar mt-3 min-h-0 flex-1 space-y-3 overflow-y-auto pr-1 xl:mt-4 xl:space-y-4">
        <section className="arena-subpanel border-white/10 bg-white/5 p-3 shadow-[0_0_20px_rgba(123,92,255,0.18)] backdrop-blur-md xl:p-4">
          <div className="text-[12px] font-medium text-white/55">发言轮次</div>
          <div className="mt-2 flex items-end justify-between gap-3">
            <div className="text-[26px] font-semibold leading-none text-white xl:text-[30px]">
              {currentRound > 3 ? 'OT' : Math.max(currentRound, 1)}
              <span className="ml-1 text-[14px] text-white/45 xl:text-[15px]">
          ...
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10 xl:mt-4">
            <div
