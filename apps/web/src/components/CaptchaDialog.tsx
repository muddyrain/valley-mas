...
export default function CaptchaDialog({ open, onOpenChange, onVerify }: CaptchaDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        showCloseButton={false}
        className={cn(
          'w-auto max-w-[min(calc(100vw-2rem),24rem)]',
      ...
      >
        <DialogTitle className="sr-only">人机验证</DialogTitle>
        <DialogDescription className="sr-only">完成验证后发送验证码</DialogDescription>
        <div
          className="relative"
          style={
            {
...
function ChineseClawCaptcha({ onVerify }: { onVerify: () => void }) {
  ...
  const restructureSub = useCallback(() => {
    ...
    // Split into: text before img → line1, img + text after img → line2
