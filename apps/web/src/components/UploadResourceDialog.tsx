    ...
    >
      <DialogContent className="flex h-[90vh] w-[90vw] max-w-5xl flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl">
        ...
        <div className="flex min-h-0 flex-1 divide-x divide-border overflow-hidden">
          {/* ── 左栏：图片拖拽 & 预览 ── */}
          <div className="flex w-[52%] shrink-0 flex-col gap-4 p-6 bg-muted/40">
            ...
            <div
              className={`relative flex-1 min-h-0 rounded-2xl border-2 border-dashed transition-all duration-200 cursor-pointer group overflow-hidden ${
                previewUrl
                  ? 'border-primary'
                  : 'border-border hover:border-primary bg-card hover:bg-accent/30'
              }`}
              onDrop={handleDrop}
            ...
            >
              {previewUrl ? (
                <div className="relative h-full w-full">
                  ...
                  />
                  {/* 底部信息条 */}
                  <div className="absolute inset-x-0 bottom-0 flex items-center justify-between rounded-b-2xl bg-foreground/50 px-3 py-2 backdrop-blur-sm">
                    <div className="flex items-center gap-2 min-w-0">
                      <ImageIcon className="h-3.5 w-3.5 text-primary-foreground/70 shrink-0" />
                      <span className="text-xs text-primary-foreground/90 truncate">
                        {uploadFile?.name}
                      </span>
                      <span className="text-xs text-primary-foreground/50 shrink-0">
                    ...
                    <button
                      ...
                      className="ml-2 shrink-0 p-1 rounded-full bg-card/20 text-primary-foreground hover:bg-card/40 transition-colors"
                    >
                      <X className="h-3 w-3" />
                    </button>
                  </div>
                  {/* 重新选择遮罩 */}
                  <div className="absolute inset-0 flex items-center justify-center rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity bg-foreground/20">
                    <span className="rounded-full bg-card/90 px-3 py-1.5 text-xs font-medium text-foreground shadow">
                      点击重新选择
                    </span>
                  </div>
                </div>
                ...
                <div className="flex h-full flex-col items-center justify-center px-4 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">JPG、PNG、WebP · 最大 30MB</p>
                </div>
              )}
            </div>
            <input
          ...
          <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {RESOURCE_TYPE_OPTIONS.map((opt) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-semibold leading-none ${uploadType === opt.value ? 'text-primary' : 'text-foreground'}`}
                      >
                        {opt.label}
                      </span>
                  ...
                  {VISIBILITY_OPTIONS.map((opt) => (
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <div
                          className={`text-xs font-semibold leading-none ${uploadVisibility === opt.value ? 'text-primary' : 'text-foreground'}`}
                        >
                          {opt.label}
                        </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-lg bg-[linear-gradient(135deg,hsl(var(--primary) / 0.10),hsl(var(--primary) / 0.06))] border border-primary/25 px-2.5 py-1 text-xs font-medium text-primary transition-all hover:bg-primary hover:text-primary-foreground hover:border-primary hover:shadow-[0_2px_8px_hsl(var(--primary) / 0.30)] disabled:opacity-35 disabled:cursor-not-allowed"
                ...
                {aiTitles.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 pt-0.5">
                    ...
                    <button
                      type="button"
                      onClick={() => setAiTitles([])}
                      className="rounded-full border border-border bg-muted px-2.5 py-1 text-xs text-muted-foreground hover:bg-accent transition-all"
                    >
                      清除
                    </button>
                  </div>
            ...
            {/* 底部操作栏 */}
            <div className="shrink-0 border-t border-border bg-muted/60 px-6 py-4">
              {uploadConfirming && (
                <div className="mb-3 rounded-xl border border-primary/30 bg-accent px-3 py-2 text-xs text-primary">
                  {uploadConfirmingMessage}
                </div>
              )}
