    ...
    <header class="site-header">
      <div class="header-inner">
        ...
        <nav class="desktop-nav" aria-label="主要导航">
          ...
          <button class="nav-link ${activeView === 'article' ? 'is-active' : ''}" data-view="article">文章</button>
          <button class="nav-link" data-section="gallery">图库</button>
          <button class="nav-link" data-section="about">关于</button>
        </nav>
      ...
      <nav class="mobile-nav ${prototypeState.mobileNavOpen ? 'is-open' : ''}" aria-label="移动端导航">
        ...
        <button data-view="article">文章</button>
        <button data-section="gallery">图库</button>
        <button data-section="about">关于</button>
...
function imageFigure(image, className = '') {
  return `
    <figure class="editorial-image ${className}">
      <button class="image-button" data-action="image-detail" data-image="${image.title}" aria-label="查看${image.title}">
    ...
    <main id="main-content" class="public-main home-page">
      ...
      <section class="recent-section" aria-labelledby="recent-title">
        <header class="section-heading">
          ...
          <button class="underlined-action" data-view="article">查看全部</button>
        </header>
      ...
      <section class="gallery-section" id="gallery" aria-labelledby="gallery-title">
        ...
        <div class="image-composition">
          ${imageFigure(contentData.images[0], 'image-wide')}
          ${imageFigure(contentData.images[1], 'image-tall')}
          ${imageFigure(contentData.images[2], 'image-small')}
          ${imageFigure(contentData.images[3], 'image-offset')}
        </div>
      </section>
      ...
      <section class="about-strip" id="about" aria-labelledby="about-title">
        ...
        <a class="about-link" href="https://github.com/muddyrain" target="_blank" rel="noreferrer">GitHub ↗</a>
      </section>
    </main>
    ${publicFooter()}
  `;
    ...
    <main id="main-content" class="studio-shell">
      ...
      <div class="studio-layout ${prototypeState.publishOpen ? 'publish-open' : ''}">
        <aside class="task-rail" aria-label="创作室导航">
          <button class="rail-brand" aria-label="创作室首页">雨</button>
          <nav>
            <button class="rail-item is-active"><span>写</span><small>文章</small></button>
            <button class="rail-item"><span>图</span><small>图库</small></button>
            <button class="rail-item"><span>像</span><small>生图</small></button>
          </nav>
        ...
        <aside class="publish-drawer" aria-label="发布检查">
          ...
          <button class="publish-button">发布文章</button>
...
function publicFooter() {
  return `
    <footer class="site-footer">
      <div>${wordmark()}</div>
      <p>雨迹 · by @muddyrain</p>
      <nav aria-label="页脚导航"><button data-view="home">首页</button><button data-view="article">文章</button><button data-section="gallery">图库</button></nav>
    </footer>
  `;
}
...
function searchDialog() {
  ...
  const searchable = [contentData.featured, ...contentData.articles];
  const results = searchable.filter(
    (article) => !term || article.title.toLowerCase().includes(term),
  );
  return `
    <div class="dialog-backdrop" data-action="close-search">
      <section class="search-dialog" role="dialog" aria-modal="true" aria-labelledby="search-title" data-dialog>
        ...
        <label class="search-field"><span>关键词</span><input autofocus value="${prototypeState.searchTerm}" data-search-input placeholder="React、LangGraph…" /></label>
          ...
          ${
            results.length
              ? results
                  ...
                  .map(
                    (article) =>
                      `<button data-view="article"><span>${article.column}</span><strong>${article.title}</strong><small>${article.date}</small></button>`,
                  )
                  .join('')
              : '<p class="empty-search">没有找到相关文章</p>'
          }
        </div>
      </section>
    </div>
    ...
    ${
      prototypeState.viewsOpen
        ? `<aside class="views-panel prototype-panel" aria-label="原型页面">
            ...
            <button data-view="article"><span>02</span>文章详情</button>
            <button data-view="studio"><span>03</span>内容画布</button>
          </aside>`
        : ''
    }
  `;
...
function normalizeView(view) {
  return ['home', 'article', 'studio'].includes(view) ? view : 'home';
}
...
function render() {
  ...
  const viewMarkup =
    prototypeState.view === 'article'
      ? articleView()
      : prototypeState.view === 'studio'
        ? studioView()
        : homeView();
...
function goToSection(sectionId) {
  if (prototypeState.view !== 'home') {
    prototypeState.view = 'home';
  ...
  window.requestAnimationFrame(() => {
    ...
    window.scrollTo({
      ...
      behavior: prototypeState.motion ? 'smooth' : 'auto',
    });
  });
}
...
document.addEventListener('click', (event) => {
  ...
  if (target.dataset.section) {
    event.preventDefault();
    prototypeState.mobileNavOpen = false;
    goToSection(target.dataset.section);
    return;
  }
  ...
  if (action === 'image-detail') {
    const toast = document.querySelector('[data-image-toast]');
    toast.textContent = `${target.dataset.image} · 已选中`;
    toast.hidden = false;
    window.setTimeout(() => {
      toast.hidden = true;
    }, 2200);
