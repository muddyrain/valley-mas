...
- [x] 调研玩具风格角色模型（CC0 / 免费角色资源仅作参考，最终采用原创制作）
- [x] 制作或采购玩具人偶角色 GLB 模型（新增原创 `toy_hero.glb`）
- [x] 集成新角色（木偶，程序化，无需 GLB，含跑/跳/落地动画）
- [x] 新增角色选择界面（独立角色选择台，支持点击切换与大预览）
...
- [~] 拆分并扩展程序化玩具平台模型生成器
- [x] 补齐静态基础平台资产变体（S1-S6 已完成首批 GLB 并接入主路线）
