...
- Launchpad v1 已接入 Dock 和菜单栏，并升级为 macOS 式覆盖层：支持关闭按钮、搜索、左右分页、分页指示点、打开 App 后自动关闭，以及基于 Motion 的打开/关闭和翻页动画。
...
- Mail v1 已接入只读统一收件箱：Gmail 通过 OAuth / Gmail API 绑定，QQ 邮箱通过授权码 / IMAP 绑定；服务端加密保存外部邮箱凭据，Desktop OS 只展示账号、邮件摘要、详情预览、搜索、解绑和手动刷新，不做发信、回复、附件下载或邮件状态修改。
- Weather App 城市侧栏已调整为浏览优先：默认只展示当前位置和用户添加城市，并隐藏搜索框和删除按钮；当前位置带定位图标，可重新请求浏览器定位；通过侧栏城市设置面板添加城市、删除已添加城市，当前位置固定保留；已登录用户的添加城市列表通过 user preference 云端同步，未登录时保留本地。
...
- Mini Apps v3 已新增工具箱型 App：开发工具箱覆盖 JSON、时间戳、编码、ID/哈希、Diff 和 CSV，日常工具箱覆盖日期、密码、图片和分账；两者默认在 Launchpad/Spotlight 中发现，不默认进入 Dock。
- Mini Apps v3 的通用工具逻辑已抽离到 `@valley/format-tools`：JSON、Query、时间戳、编码、JWT、Hash、随机 ID、CSV、Diff、日期、密码和分账作为 monorepo 纯函数能力复用；图片元数据读取、压缩、改宽和导出归属 `@valley/browser-media`，Desktop OS 只保留窗口 UI、本地草稿、剪贴板和图片工具交互编排。
...
- AI Command Center v1 已作为 Desktop OS 原生窗口接入，复用现有 `/ai/chat` 服务端能力，提供 Chat、总结、翻译、改写、Prompt Lab 快捷指令和本地会话历史；布局调整为左侧会话列表、右侧对话流、底部输入框的主流 AI Chat 结构，当前不新增 server API、不接图片生成、不做本地 agent 自动化。
...
- 工具箱 v3 保持本地运行，不新增第三方依赖；通用纯函数归属 `@valley/format-tools`，浏览器图片处理归属 `@valley/browser-media`，密码结果和图片处理结果不持久化，图片只通过浏览器本地 canvas 处理。
...
- AI Command Center 作为 Dock、Launchpad 和 Spotlight 可打开的真实窗口；第一阶段只接文本类 `/ai/chat`，聚焦总结、翻译、改写、Prompt Lab、通用问答和本地会话历史，不再推进 3D 宠物或宠物工坊。
...
3. Safari v3：增强 iframe 状态、最近访问、起始页分组和可选标签页。
...
7. 通知中心 + 小组件 v2：资源动态、登录态、更多真实数据源和可配置小组件。
8. Notes v2：评估是否从 Life Trace Inbox 迁移到独立 `/user/notes` 数据模型。
9. AI Command Center v2：评估云端会话同步、常用指令收藏、流式输出、跨 App 选中文本发送、图片工具和更明确的任务型 Agent 编排。
...
- Launchpad 支持左右分页、分页指示点、上一页/下一页按钮和键盘左右翻页；375px、768px、1024px、1440px 下不横向溢出。
...
- AI Command Center 使用类似 ChatGPT/Gemini/豆包的对话布局：左侧展示本地会话历史，右侧展示当前对话流，底部输入框支持 Enter 发送、Shift+Enter 换行。
- AI Command Center 可新建、切换、删除和清空本地会话；刷新页面后本地会话历史仍保留。
- AI Command Center 登录后可调用 `/ai/chat`，支持 Chat、总结、翻译、改写和 Prompt Lab 五种模式；未登录时显示“登录后使用”，不会发起请求。
...
- Mail 窗口登录后展示统一收件箱、账号侧栏、搜索框、邮件列表、详情预览和手动刷新；未登录或未绑定邮箱时显示空状态，不发起无效同步。
- Mail v1 只读外部收件箱，不提供发信、回复、附件下载、已读/星标/归档状态修改。
- Weather App 默认城市列表只用于切换城市，只展示当前位置和用户添加城市，不展示搜索框或删除按钮；当前位置行显示定位图标并可重新定位，定位失败时不清空原天气；城市设置面板可添加城市、删除已添加城市，当前位置不可删除；刷新页面后已添加城市不会丢失，登录后可从云端偏好恢复。
...
- Launchpad 能展示并打开开发工具箱和日常工具箱，Spotlight 搜索 JSON、时间戳、Base64、UUID、CSV、密码、日期、图片和分账能命中工具箱。
- 开发工具箱可完成 JSON 格式化/压缩/Query 转换、时间戳互转、Base64/URL/JWT 解码、UUID/token/密码/SHA 生成、文本 Diff 和 CSV/JSON/Markdown table 转换。
- 日常工具箱可完成日期差和日期加减、密码生成、本地图片压缩/改宽/导出，以及多人分账计算。
- 密码生成结果刷新后不保留；图片工具只处理本地文件，不上传 server。
...
- `pnpm --filter @valley/format-tools typecheck`、`pnpm --filter @valley/format-tools check` 和 `pnpm --filter @valley/format-tools build` 通过。
