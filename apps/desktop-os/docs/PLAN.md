...
- Storybook Design System v1 已明确 shadcn 主干化边界：`src/components/ui/*` 保持为 shadcn/base-ui 源码层，业务窗口优先通过 `src/ui/PlushPrimitives.tsx` 消费 Plush 视觉皮肤；公共 token 已补齐控件高度、文字层级、info/lagoon 语义和 ember/sunny/lagoon/meadow/petal Identity accent。
- Desktop OS 壳层已收敛到 Neutral Light 主题皮肤：`theme.css` 新增 `--system-*` neutral 语义 token，并把 shadcn `--background`、`--foreground`、`--card`、`--popover`、`--primary`、`--muted`、`--accent`、`--border`、`--input`、`--ring` 映射到浅色中性系统层；Window、MenuBar、Dock、Launchpad、Spotlight、Control Center 和 Notification Center 的大面积底色、标题栏、hover、popover、focus ring 和阴影优先使用 system neutral，Plush 原色退回 Scene、Identity、状态和特殊业务点缀。
- Desktop OS App 内部已启动 Neutral Light 公共样式收敛：`theme.css` 新增 `--app-*` 业务窗口语义 token，`PlushPrimitives.css` 新增 `.plush-app-panel`、`.plush-app-card`、`.plush-app-toolbar`、`.plush-app-field`、`.plush-app-action` 公共类；公共 Plush 控件和 Finder、Blog、Weather、Notes、Calendar、Safari、About、DockAppWindows、MiniApps 的第一批面板/卡片/字段/按钮/选中态已改为优先消费 app neutral token。
- Blog App 已作为 shadcn 主干化样板窗口启动迁移：阅读/创作模式、搜索工具栏、创作侧栏卡片、主要按钮、输入框、标签和正文链接优先走 Plush 原语与 token，不再依赖未定义的私有 `--plush-water-deep`。
...
- Calendar App v1 已接入 `@valley/calendar` 子包：事件本地状态、法定节假日、周末休息、补班/调休和农历信息均由子包提供，Desktop OS 窗口只负责月/周/日视图、日程表单、提醒通知和 Plush 风格呈现。
...
- Blog App v1 已接入 server 博客能力：默认进入 Dock、Launchpad 和 Spotlight，阅读模式使用 `/public/blog/*` 读取公开已发布博客列表、分组、分类、标签和详情，提供搜索、筛选、排序、刷新、加载更多和 `react-markdown` + `remark-gfm` 正文阅读；创作模式复用现有 `/admin/blog/posts` 与 `/admin/blog/cover/upload`，登录 creator/admin 后可新增博客、导入 Markdown、选择标签、选择封面、配置编辑器视图/字号/自动摘要/换行，并保存草稿或发布公开博客；当前不提供删除、评论、AI 导读或后台管理入口。
...
- Desktop OS 已完成运行时生命周期治理：窗口内容、音乐 audio runtime、通知中心/控制中心面板和资源数据加载均按打开或使用时激活，窗口渲染器通过 `React.lazy` 按 App 拆分加载，窗口焦点提供轻量 `focusedAppId`，拖拽和缩放按 animation frame 合并写入，保留动画和毛玻璃效果但减少关闭态、首屏主包和高频交互开销。
- Desktop OS 已完成后台任务与 App 独立运行治理：普通 App 默认 `foreground-only`，最小化后只保留窗口壳层和状态元数据，业务组件、定时器和局部监听随窗口内容卸载；Music 与 FocusTimer 作为明确后台白名单运行，但 runtime gate 只在窗口仍运行或存在播放/倒计时等真实后台任务时挂载，根组件的全局事件和通知轮询集中到 Gate，Spotlight/Safari/Finder 的资源加载和滚动持久化避开首帧与滚动热路径。
...
- Blog App 作为 Dock、Launchpad 和 Spotlight 可打开的真实窗口；第一阶段阅读公开已发布博客，创作侧只复用现有 server 后台接口新增博客和上传封面，不新增 server API，正文使用 `react-markdown` + `remark-gfm` 渲染 Markdown，不渲染原始 HTML。
...
- 当前 System 层视觉方向采用 shadcn neutral light + Desktop OS Plush 包装层：壳层优先白、近白、neutral 灰、黑色文字、细边框和轻阴影；壁纸、图标、危险/成功/信息状态和 Identity 装饰继续允许使用少量 Plush 原色。
- 当前 App 层视觉方向采用 `--app-*` neutral token + Plush 公共类：业务窗口的 toolbar、panel、card、field、button、empty/loading 应优先复用公共层;天气图标、游戏棋盘、农场场景、骰盅舞台、头像和状态反馈允许保留小面积 Identity/Scene 色。
- 公共滚动体验由 `PlushScrollbar` 封装第三方 overlay scrollbar 能力，应用窗口和高频滚动面板优先接入该组件，不直接暴露浏览器默认滚动条或第三方默认主题。

## 下一步顺序

1. Storybook Design System v1：以 Blog 样板为基准，继续把 Finder、Safari、Weather、Music 的按钮、输入、菜单、弹窗、卡片、空状态、加载态和高频窗口入口迁到 `PlushPrimitives`，保持 shadcn 作为组件主干、Plush 作为视觉皮肤。
...
- Calendar 月/周/日视图能显示来自 `@valley/calendar` 的法定节假日、周末休息、补班/调休和农历信息；右侧日期详情展示农历标签，春节、端午、中秋、除夕等农历节日优先显示。
...
- Blog 窗口展示公开已发布博客列表、封面、摘要、作者/时间/浏览量、分组、分类、标签、搜索框、筛选、排序、刷新和加载更多；整体界面采用 Typora 式文档阅读结构，顶部保留紧凑工具栏，左侧维护独立且可折叠的阅读队列滚动区，中间是低干扰文章纸面，右侧只保留轻量大纲面板，色彩跟随 Desktop OS 毛绒主题。
- Blog 详情以 `react-markdown` + `remark-gfm` 展示 Markdown 正文，不使用 `dangerouslySetInnerHTML`。
- Blog 创作模式登录 creator/admin 后可新增博客、导入 `.md/.markdown` 文件、选择标签、选择本地封面上传、填写标题/摘要/正文，支持分屏/编辑/预览、字号和自动摘要/换行配置，并可保存草稿或发布公开博客。
- Blog 创作模式不提供删除、批量管理、后台管理、评论或 AI 导读入口；未登录或非 creator/admin 用户只看到权限提示，不影响公开阅读。
...
- Window 标题栏、Window body、MenuBar 菜单、Dock 背板/tooltip/右键菜单、Launchpad overlay、Spotlight 面板、Control Center 和 Notification Center 的大面积系统层颜色需来自 `--system-*` neutral token；不能重新出现明显奶黄渐变、强黄/蓝/绿氛围光、默认蓝色 hover 或裸 rgba 主色。
- App 内部大面积面板、卡片、工具栏、字段、列表选中态和普通按钮需来自 `--app-*` token、`PlushPrimitives` 或 `.plush-app-*` 公共类；新增 App 不应复制私有浅暖色 panel/card/button/input 配方。Scene/Identity 资产、天气状态、游戏画面和危险/成功/信息小状态色除外。
