...
export default function MusicMenuItem() {
    ...
    <div ref={rootRef} className="menu-music" onPointerDown={(event) => event.stopPropagation()}>
      ...
      >
        ...
        <span className="menu-music__trigger-text">
          ...
          <span>{isPlaying ? '播放中' : track.mood}</span>
        </span>
      </button>
      <button
        type="button"
      ...
      {isOpen ? (
        <div className="menu-music-panel" role="dialog" aria-label="音乐播放器">
          ...
          <div className="menu-music-panel__controls">
            <button type="button" onClick={() => previousTrack(true)} aria-label="上一首">
              上一首
            </button>
            <button type="button" className="is-primary" onClick={togglePlay}>
              {isPlaying ? '暂停' : '播放'}
            </button>
            <button type="button" onClick={() => nextTrack(true)} aria-label="下一首">
              下一首
            </button>
          </div>
