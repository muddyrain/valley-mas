...
static func build_mesh() -> ArrayMesh:
	...
	for axis: Vector3 in [Vector3.RIGHT, Vector3.UP]:
		var corners: Array[Vector3] = [Vector3.ZERO, axis * .12 + Vector3(0, 0, -.10), Vector3(0, 0, -.37), -axis * .12 + Vector3(0, 0, -.10)]
		for index in 4:
	...
	for index in 8:
		...
		var radius := .13 if index % 2 == 0 else .055
		var next_radius := .13 if (index + 1) % 2 == 0 else .055
		surface.set_color(Color(1.0, .96, .75, 1.0))
