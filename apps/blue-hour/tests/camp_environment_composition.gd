...
func run() -> void:
	...
	output_directory = "res://test-output/camp-environment-composition/" + label + "/"
	...
	app.save_path = "user://test-runs/camp-environment-composition.json"
	...
	measurements["vegetation"] = {"trees": 0, "bushes": 0}
	for plant: Node3D in camp.get_node("CampDressing/BoundaryPlanting").get_children():
		var kind: String = "trees" if "tree" in str(plant.get("asset_id")) else "bushes"
		measurements.vegetation[kind] += 1
	...
	await shot("camp_env_comp_02b_A_hud")
	app.ui_layer.hide()
	...
	for node: Label3D in camp.find_children("*", "Label3D", true, false):
		node.hide()
	await frames(3)
	await shot("camp_env_comp_02b_B_no_hud")
	await RenderingServer.frame_post_draw
	var still := root.get_texture().get_image()
	_crop(still, camera, "C_left_boundary", Vector3(-10.2, 0.5, -3.8), Vector2i(800, 880))
	_crop(still, camera, "D_right_boundary", Vector3(10.0, 0.5, 0.5), Vector2i(860, 900))
	_crop(still, camera, "E_rest_area", Vector3(1.0, 0.3, 1.7), Vector2i(780, 650))
	check(still.save_png(output_directory + "camp_env_comp_02b_F_overview.png") == OK, "Same-camera overview")
...
func _crop(still: Image, camera: Camera3D, id: String, center: Vector3, size: Vector2i) -> void:
	...
	check(still.get_region(region).save_png(output_directory + "camp_env_comp_02b_" + id + ".png") == OK, "Formal camera crop: " + id)
