...
const CLIP: String = "res://assets/characters/survivor_animation_template/animations/running.tres"
const OUT: String = "res://test-output/standard-survivor-running-reconstruction/runtime-contact.json"
...
func run() -> void:
	...
	library.add_animation("Running", clip)
	player.add_animation_library("", library)
	player.callback_mode_process = AnimationMixer.ANIMATION_CALLBACK_MODE_PROCESS_MANUAL
	player.play("Running")
	player.advance(0.0)
	check(is_equal_approx(clip.length, 16.0 / 24.0), "Original Running duration")
	...
	for frame: int in 81:
		...
		for side: String in ["Left", "Right"]:
			var strike: float = 0.1833333333333333 + (clip.length / 2.0 if side == "Right" else 0.0)
			var phase: float = fposmod(time - strike, clip.length)
			if phase <= 0.2333333333333333 + 0.00001:
				if phase <= 0.1583333333333333 + 0.00001:
					check(absf(feet[side].heel) <= 0.007, side + " support heel within 7 mm")
				if phase >= 1.0 / 24.0 - 0.00001:
					check(absf(feet[side].forefoot) <= 0.007, side + " support forefoot within 7 mm")
	...
	file.close()
	print("STANDARD RUNNING CONTACT: ", checks, " checks; failures=", failures)
	actor.queue_free()
	await process_frame
