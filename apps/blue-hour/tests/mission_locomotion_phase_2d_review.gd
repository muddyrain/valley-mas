extends SceneTree
## Formal Mission fixture. Only commands and the runtime cadence choice differ from play.
...
const OUTPUT := "res://test-output/locomotion-phase-2d/"
var mission: Node3D
var actor: Node3D
var profile: StringName = &"current"
...
func run() -> void:
	...
	FileAccess.open(OUTPUT + "jog-cadence-" + profile + ".json", FileAccess.WRITE).store_string(JSON.stringify(report, "\t"))
	print("PHASE 2D FILM ", profile, ": ", samples.size(), " frames; checks=", checks, "; turns=", max_turn, "; failures=", failures)
	hud.free()
	mission.free()
	await process_frame
...
func record(segment: String, frames: int) -> void:
	for frame in frames:
		...
		samples.append({"frame": samples.size(), "segment": segment, "position": [position.x, position.y, position.z],
			...
			"lean": layer.lean_degrees, "yaw": actor.rig.rotation.y, "feet": feet})
		...
		if DisplayServer.get_name() != "headless" and ((key in ["JogStart", "JogStop"] and count % 3 == 0) or (key in ["turn-90", "s-left", "s-right", "turn-180", "straight-final", "idle-final"] and count in [3, 9, 15, 90])):
			...
			root.get_texture().get_image().save_png(OUTPUT + String(profile) + "-" + key + "-%03d.png" % count)
