extends SceneTree
## Formal camera, HUD, route and gameplay clock for the Phase 2E arm comparison.
...
var label: String = "unarmed-arm-swing-v2"
...
func run() -> void:
	...
	actor.animation_controller.arm_swing_v2_enabled = not label.begins_with("unarmed-arm-swing-v1")
	...
	check(actor.weapon == null and actor.data.id == character, "Requested unarmed character")
	...
	for sample in samples:
		...
		if not previous.is_empty() and sample.speed > actor.data.move_speed * .94 and previous.speed > actor.data.move_speed * .94:
	...
	var report := {"character": character, "profile": String(profile), "native": DisplayServer.get_name() != "headless", "fps": 60,
	...
	print("PHASE 2E UNARMED FILM ", label, ": ", samples.size(), " frames; checks=", checks, "; turns=", max_turn, "; failures=", failures)
	hud.free()
	mission.free()
	await process_frame
