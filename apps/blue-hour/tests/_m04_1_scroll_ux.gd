...
func run() -> void:
	...
	_check(app.campaign.discover_survivor("SUR_012"), "SUR_012 can be discovered before selection check")
	_check(app.campaign.recruit_survivor("SUR_012"), "SUR_012 can be recruited before selection check")
	roster.refresh()
	await _frames(3)
	scroll.scroll_vertical = int(scroll.get_v_scroll_bar().max_value)
	await _frames(3)
	_check(not roster.get_survivor_view_data("SUR_012").is_empty(), "Scrolled roster keeps the SUR_012 view available")
