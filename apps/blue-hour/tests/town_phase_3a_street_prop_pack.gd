...
func _run() -> void:
	...
	for asset_id: String in IDS:
		var path := "res://assets/world/props/street_prop_pack_v1/%s.glb" % asset_id
	...
	FileAccess.open(OUTPUT.path_join("acceptance-report.json"), FileAccess.WRITE).store_string(JSON.stringify({"checks": checks, "failures": failures, "assets": IDS, "seeds": records, "blender_available": false}, "\t"))
...
func _audit_seed(seed_value: int) -> void:
	...
	for item: Dictionary in result.instances:
		for road: Dictionary in town.roads:
			_check(not item.bounds.intersects(road.bounds), "Seed %d prop clears roads" % seed_value)
		for site: Dictionary in town.buildings:
			_check(not item.bounds.intersects(site.bounds.grow(0.18)), "Seed %d prop clears buildings" % seed_value)
	print("PHASE_3A seed=%d instances=%d zones=%s arrival=%d" % [seed_value, result.instances.size(), str(result.by_zone), arrival.size()])
	records.append({"seed": seed_value, "instances": result.instances.size(), "by_zone": result.by_zone, "arrival": arrival.size(), "signature": var_to_str(result).sha256_text()})
