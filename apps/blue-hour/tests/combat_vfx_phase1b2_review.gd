extends SceneTree
## Fixed-camera capture harness for the Combat VFX Phase 1B.2 polish pass.
...
const OUTPUT_DIR := "res://test-output/combat_vfx_phase1b2"
const FPS := 15
...
func run() -> void:
	...
	FileAccess.open(OUTPUT_DIR.path_join("review.json"), FileAccess.WRITE).store_string(JSON.stringify(result, "\t"))
	print("COMBAT VFX PHASE 1B.2 REVIEW: %d frames, %d screenshots, %d failures" % [frame_index, screenshot_index, failures.size()])
	mission.queue_free()
	await process_frame
	quit(0 if failures.is_empty() else 1)

func _stage_enemies() -> void:
	for offset: Vector3 in [Vector3(-1.15, 0, -2.4), Vector3(-0.55, 0, -3.1), Vector3(0.0, 0, -3.35), Vector3(0.55, 0, -3.1), Vector3(1.15, 0, -2.4)]:
...
func _capture_sequence() -> void:
	...
	for shot in 5:
		_fire_at(enemies[2])
		if shot == 1:
			await _capture("01_muzzle_flash.png")
		await _review_wait(0.45)
	shooter.equip(_weapon(Registry.A21))
	for shot in 14:
		_fire_at(enemies[4])
		if shot == 6:
			await _capture("02_tracer.png")
	...
	for shot in 3:
		_fire_at(enemies[2])
		if shot == 1:
			await _capture("03_hit_effect.png")
		await _review_wait(0.85)
	shooter.equip(_weapon(Registry.P9))
	for cycle in 8:
		_fire_at(enemies[cycle % enemies.size()])
		if cycle == 3:
			await _capture("04_enemy_feedback.png")
...
func _review_wait(seconds: float) -> void:
	for _frame in maxi(1, roundi(seconds * FPS)):
		...
		if DisplayServer.get_name() != "headless":
			...
			root.get_texture().get_image().save_png(OUTPUT_DIR.path_join("frames/frame_%04d.png" % frame_index))
		frame_index += 1
