...
func _initialize() -> void:
	...
	var early_hp: float = clock.hp_multiplier()
	var early_damage: float = clock.damage_multiplier()
	var early_interval: float = clock.spawn_interval()
	clock.advance(90.0)
	check(clock.threat_level() > 1, "Night threat grows with time")
	check(clock.hp_multiplier() > early_hp and clock.damage_multiplier() > early_damage, "Staying at night scales HP and damage")
