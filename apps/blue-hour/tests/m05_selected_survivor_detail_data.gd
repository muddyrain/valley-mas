...
func run() -> void:
	...
	_check(detail.get_node("HeaderPanel/TraitBadge/Label").text == "探索专家", "M05 summarizes the selected survivor's mission role")
	_check(detail.has_node("HeaderPanel/PortraitContainer/HalfPortrait"), "M05 keeps an independent portrait container")
	_check(not detail.get_node("BackgroundPanel").visible and not detail.get_node("HeaderPanel/RoleLabel").visible, "M05 keeps the mission header free of profile copy")
	_check(detail.get_node("CombatPanel/EquipmentTitle").text == "装备", "M05 has a distinct equipment section")
	_check(detail.get_node("CombatPanel/WeaponIconSlot") != null, "M05 reserves an equipment icon slot")
	_check(detail.get_node("HeaderPanel/PortraitContainer/HalfPortrait").size == Vector2(162, 131), "M05 gives the selected survivor portrait stronger presence")
	...
	_check(detail.get_node("ActionBar/UpgradeButton").get_theme_stylebox("disabled").bg_color != detail.get_node("ActionBar/SwitchButton").get_theme_stylebox("disabled").bg_color, "Upgrade remains visually primary without changing its availability")
	_check(detail.get_global_rect().size == Vector2(390, 526), "M05 keeps the compact dossier panel footprint")
	...
	_check(detail.get_node("HeaderPanel/TraitBadge/Label").text == "特殊技能辅助", "SUR_012 role tag follows its survivor data")
