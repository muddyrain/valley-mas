...
func run() -> void:
	...
	_check(detail.get_node("HeaderPanel/SurvivorNameEn").text == "SUR_001 · Lv.1 · HP 100/100", "M05 formats ID, level, and HP")
	_check(detail.get_node("HeaderPanel/RoleLabel").text.contains("灾变前身份") and detail.get_node("HeaderPanel/RoleLabel").text.contains("城市徒步"), "M05 uses background title")
	_check(detail.get_node("HeaderPanel/Quote").text == "她以前就喜欢钻进城市那些不太有人注意的小巷、旧街区和废弃建筑，习惯用相机记录路线和有趣的小角落。灾变以后，这种对环境异常敏感的习惯让她总能更快找到入口、储藏间以及容易被遗漏的物资。", "M05 uses background description")
