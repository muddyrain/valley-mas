...
func show_survivor(data: Dictionary) -> void:
	...
	$HeaderPanel/SurvivorNameEn.text = "%s · Lv.%d · HP %d/%d" % [
		str(data.get("id", "")),
		int(data.get("level", 1)),
		roundi(float(data.get("hp", 0.0))),
		roundi(float(data.get("max_hp", 0.0))),
	]
	$HeaderPanel/RoleLabel.text = "灾变前身份 · " + str(data.get("background_title", ""))
	$HeaderPanel/Tags.text = str(data.get("tags", ""))
	$HeaderPanel/Quote.text = str(data.get("background_description", ""))
