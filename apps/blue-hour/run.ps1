...
switch ($Mode) {
    ...
    'test' {
        ...
        Invoke-Engine @('--headless', '--script', 'tests/mission_jog_v2.gd')
        Invoke-Engine @('--headless', '--script', 'tests/mission_locomotion_phase_2d.gd')
        Invoke-Engine @('--headless', '--script', 'tests/mission_jog_v2_2.gd')
        Invoke-Engine @('--headless', '--script', 'tests/upper_body_phase_2e.gd')
        Invoke-Engine @('--headless', '--script', 'tests/combat_jog_phase_2e.gd')
    ...
    'build' {
        ...
        $buildInfo = [ordered]@{
            ...
            missionStyleChecks = @('mission-locomotion-style-headless', 'mission-locomotion-style-native', 'shared-mission-jog', 'muzzle-flash')
            campStyleChecks = @('camp-locomotion-style', 'camp-walk-context')
            locomotionPhase2DChecks = @('mission-locomotion-phase-2d')
            jogV22Checks = @('mission-jog-v2_2-locked-body-support', 'cadence-c-30-60-144fps', 'start-endpoint')
            upperBodyPhase2EChecks = @('upper-body-phase-2e-locked-assets', 'combat-jog-phase-2e-shared-clock-grip-aim-transitions')
