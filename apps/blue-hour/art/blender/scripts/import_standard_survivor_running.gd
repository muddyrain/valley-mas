...
const INPUT: String = "res://test-output/standard-survivor-running-reconstruction/baked-pose.json"
const OUTPUT: String = "res://assets/characters/survivor_animation_template/animations/running.tres"
const EVIDENCE: String = "res://test-output/standard-survivor-running-reconstruction/animation-export.json"
...
func run() -> void:
	...
	var original: Animation = player.get_animation("Running")
	var result: Animation = original.duplicate() as Animation
	result.resource_name = "Running"
	...
	for track: int in original.get_track_count():
		...
		preserved += 1
	assert(is_equal_approx(result.length, 16.0 / 24.0))
