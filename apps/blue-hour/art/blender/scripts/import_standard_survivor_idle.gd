...
const INPUT: String = "res://test-output/standard-survivor-foot-contact/baked-pose.json"
const OUTPUT: String = "res://assets/characters/survivor_animation_template/animations/idle.tres"
const EVIDENCE: String = "res://test-output/standard-survivor-foot-contact/animation-export.json"
...
func run() -> void:
	...
	var original: Animation = player.get_animation("Idle_4")
	var result: Animation = original.duplicate() as Animation
	result.resource_name = "Idle_4"
	...
	for track: int in original.get_track_count():
		...
		preserved += 1
	assert(is_equal_approx(result.length, 14.0))
