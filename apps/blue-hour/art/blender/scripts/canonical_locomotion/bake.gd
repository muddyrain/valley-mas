...
const INPUT: String = "res://"
const OUTPUT: String = "res://assets/"
const TARGET: String = "res://candidates/canonical.glb"
...
func run() -> void:
	...
	var skeleton_path: String = "Skeleton3D"
	...
	for name: String in ["idle", "walking", "running"]:
		...
		clip.set_meta("retarget_profile", "canonical_public_locomotion")
		var nominal_speed: float = data.stride_ratio * ({"idle": 0.0, "walking": 1.425, "running": 2.57137}[name] as float)
		if FileAccess.file_exists("res://canonical-qa.json"):
			var qa: Dictionary = JSON.parse_string(FileAccess.get_file_as_string("res://canonical-qa.json"))
			nominal_speed = qa.clips[name].canonical.mean_support_speed_m_s
		clip.set_meta("nominal_speed", nominal_speed)
		...
		assert(ResourceSaver.save(clip, OUTPUT + "public_" + name + ".tres", ResourceSaver.FLAG_CHANGE_PATH) == OK)
		clip.take_over_path(OUTPUT + "public_" + name + ".tres")
		library.add_animation(StringName("public_" + name), clip)
		evidence[name] = {"length": clip.length, "tracks": clip.get_track_count(), "samples": data.samples.size(), "source_sha256": data.source_sha256, "output": OUTPUT + "public_" + name + ".tres"}
		print("RETARGET IMPORT ", name, ": ", evidence[name])
	assert(ResourceSaver.save(library, OUTPUT + "public_locomotion.tres") == OK)
