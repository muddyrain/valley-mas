...
- 开局选择只持有草稿，确认并成功保存后才创建 Run。版本 2 的成员 ID 引用角色模板与局内等级；版本 1 在读取验证入口兼容，保留旧三人队伍和 `.v1.bak`。起始池、训练成本、专精/被动/特殊能力来自 `data/new_run.tres`、`data/specializations/` 与 `data/effects/`；能力每日一次、暂停冻结持续时间，临时效果不得改共享模板。专项验证为 `tests/new_run.gd`、`tests/new_run_flow.gd` 和 `tests/new_run_runtime.gd`。
