...
- Implemented:
  ...
  - `observe:growth` also prints satellite-expansion diagnostics for first expansion status tick/plan, first `prepare_expansion` tick, `expansionLead`, satellite-founded tick, parent population/housing/resource snapshots, and child population/housing after founding.
...
- Planned implementation slices:
  ...
  - `PR-12F.4` Expansion preparation: initial event and inspection reason slice is implemented; next manually verify whether the map/readout gives enough lead time before satellite founding, then expand reasons only if the story is still unclear.
  - `PR-12F.5` Early-game observation harness: the first single-settlement, multi-seed, satellite-expansion, and building-site deterministic reports are implemented; next use those reports for manual frontier readability review, purpose-scored placement review, and broader expansion balance tuning.
- Non-goals:
...
27. In progress: PR-12F.5 observation harness. `observe:growth` now reports deterministic early camp-to-hamlet progression across multiple seeds, phase timing, missing chain buildings, recent growth/build events, satellite-expansion timing, and `expansionLead`; `observe:building-sites` reports multi-seed house/storage/farm/mine/barrack placement distances for purpose-scored site tuning; next use the reports for manual frontier and building-placement readability review so future balance changes are judged by story quality, not only unit tests.
