...
### 2.3 时间模型
- **Sim Tick**：固定步长（默认 1 tick = 游戏内 1 旬 / 可配置成 1 月或 1 年）。
- **Sim Speed**：UI 控制每秒执行多少 tick（0.25x / 1x / 2x / 4x / 8x / Max）。
...
5. **每 Tick 20–50 次扩张**：`runExpansionTick` 默认 `attempts = clamp(势力数 × 8, 20, 50)`，避免 3000 州下"势力一个 tick 只动一格"。
