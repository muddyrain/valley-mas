...
- **强弱修正**：`strengthBias(attacker, defender, scale)` 的 `scale` 由 `tempo.ts` 平滑输出：最大势力占比 ≤35% 时为 0.65，占比 ≥65% 时降为 0.40，中间用 `smoothstep` 插值，避免早期领地优势直接滚成速胜。
...
- **地形胜率**：平原 0.55 / 沙漠 0.55 / 森林 0.50 / 河流 0.40 / 山脉 0.32，叠加强弱修正后为最终胜率，并由 `clamp01` 限制在 2%–98%。

**战争日志可读性**：

- `capture / repel / eliminate` 事件必须带上可解释细节：节奏阶段、目标州地形、最终胜率、基础地形胜率、强弱修正、残局压力、攻守双方州数、全图占领率、战争偏好。
...
- 测试覆盖后期小国清理压力与 16x 速度档位，防止后续调参误删这两个行为。
...
- **资源 / 人口经济**：所有"实力"都从拥有的州数直接算出；地形只影响被攻击州的基础攻陷概率，不引入背包式数据。
...
- **M2 玩法闭环**：势力扩张、军队行军与战斗结算、外交关系、年表事件流、剧本注册器、随机剧本可玩。
