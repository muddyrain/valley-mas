...
| 里程碑 | 状态 | 说明 |
|---|---|---|
| M0 技术验证 | 🚧 进行中 | Vite/TypeScript 子项目骨架已创建，Phaser 场景、瓦片地图、可移动单位、基础 FSM 与生存属性原型已接入，浏览器手动验收项仍需用户确认 |
| M1 最小原型 | 🚧 进行中 | 基础 FSM、小人生存属性、资源系统、建造系统、繁殖系统、双势力启动局面、领土染色/接壤检测原型和采集阈值控制已接入；地形生成、寻路、战斗仍待实现 |
| M2 完整循环 | ⏳ 未开始 | 全职业、文明升级、外交、神力 |
| M3 内容填充 | ⏳ 未开始 | 4 种族、怪物、随机事件 |
| M4 Web 性能优化 | ⏳ 未开始 | 渲染优化、AI 分帧、Worker 批处理、压测 |

> 当前下一步：进入 TASK-014（战斗系统基础）；TASK-010 的文明阶段升级判断已可基于势力/人口数据继续补齐。
