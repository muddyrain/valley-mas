...
开发文档里的“阶段 1 / 阶段 5 / 阶段 N”只用于任务拆解和协作管理，不允许直接出现在玩家可见 UI 中。

玩家界面必须使用游戏内语言描述进度和解锁，例如：
...
玩家界面不提前展示未来内容或开发结构。未解锁的票、卡册、终局位、后续目标、下一目标和阶段编号都不要显示；达到条件后再通过电话、解锁特效或新增入口自然出现。

禁止在按钮、进度条、卡片标题、提示文本中写：
...
如果确实需要表达开发阶段，只能写在 `docs/`、任务清单或代码注释中，不写进运行时 UI 文案。
