...
2. 当改动范围跨模块、涉及既有计划，或近期上下文可能影响实现时，运行 `pnpm context:recent -- <相关目录>`；只按需读取其输出关联的文档。
...
问题与规格通过 GitHub Issues 管理。参见 `docs/agents/issue-tracker.md`。

### Triage labels

使用五个默认 triage 标签。参见 `docs/agents/triage-labels.md`。

### Domain docs

采用 multi-context 布局。参见 `docs/agents/domain.md`。
...
- 涉及文档约束（含 AGENTS、SKILL、plans、ARCHIVE/PLAN）改动，运行适用的 Harness 与文档链接校验；命令以 `docs/PROJECT_GUIDE.md` 为准。
- 不在源码、文档或示例配置中写入真实密钥。
...
- 启动或关闭长驻服务前，遵循 `docs/PROJECT_GUIDE.md` 的端口表，确认端口与进程归属；复用可用实例，不得擅自终止归属不明的进程。
- 本轮启动的服务在交付中报告实际端口与 PID，并在结束时关闭，除非用户要求保留。

## 前端与运行时证据

- 维护页面时优先复用子项目的设计系统、组件、hooks、API 封装和主题 token。
- 涉及登录态、真实数据、页面调试或视觉验收时，优先复用用户当前 Chrome 会话；不可用时使用现有自动化或等价工具，并说明未验证范围。
- 响应式、动画、Canvas、Three.js、拖拽、滚动、loading 与路由等可见行为需要风险相称的运行时证据；是否沉淀 E2E 测试由回归风险决定。

## 全局 AI Coding 约束（行为改动默认顺序）

- 对状态、权限、数据转换、请求与关键交互等行为改动，默认先更新测试或最小可复现验证，再实现并复跑受影响验证。
- 纯文案、纯样式或无可观测行为边界的改动可豁免；交付时说明未测原因与替代验证。
- 具体命令、覆盖率门槛、平台验收与例外由子项目 `AGENTS.md` 和 `docs/PROJECT_GUIDE.md` 定义；不要从根规则推导固定命令。

## 新增子项目 AGENTS 的复用模板（可直接套用）

创建或重写子项目 `AGENTS.md` 时，以 [AGENTS_TEMPLATE.md](./.agents/AGENTS_TEMPLATE.md) 为起点，并将实际命令、行为测试与平台验收要求写在子项目内。
