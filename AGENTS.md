...
- 启动 Web、Admin、Go API 或任一子项目开发服务前，先查看 `docs/PROJECT_GUIDE.md` 的端口表，并用系统端口/进程查询确认目标端口、实际监听 PID、启动命令和所属工作目录。
- 若同一项目已经存在可用实例，必须复用该实例，不得再次启动；不得把 Vite 或 Go 的自动端口顺延当作默认方案，否则前端代理、浏览器页面和后端版本容易错配。
- 目标端口被占用但进程归属不明时，先定位 PID、父子进程、可执行路径和命令行。未经用户授权不得终止既有进程；需要新端口时必须明确说明原因、实际端口，并同步代理或 `VITE_API_BASE_URL`。
- Agent 启动长驻开发服务后，必须记录并在进度或交付中报告服务、PID 和实际端口；任务结束默认关闭该 Agent 本轮启动的进程，除非用户明确要求保留。
- 关闭重复或过期服务时，只终止已确认属于当前仓库的精确进程树，随后复查端口已经释放；禁止按 `node`、`go`、`air` 等通用进程名批量结束进程。

## 前端与运行时证据

- 维护现有页面时优先使用子项目设计系统、组件、hooks、API 封装和主题 token。
- 允许使用当前环境可用的浏览器工具做本地运行时取证。
- 查看或调试页面时，优先使用 `chrome:control-chrome` 复用用户当前已打开的 Chrome 会话。涉及登录态、真实数据、页面调试或视觉验收时，必须使用该 Chrome 会话；未经用户明确同意，不得以隔离 Playwright Browser、headless 浏览器或其他独立浏览器作为替代。Chrome 不可用时，可使用仓库已有的 E2E、headless 浏览器或当前环境提供的等价工具；不要为一次验证擅自新增浏览器依赖或启动会干扰用户会话的重复 GUI 实例。仍无法取证时，再说明未验证项并提供人工验收标准。
...
## 全局 AI Coding 约束（行为改动默认顺序）

- 行为改动（状态变更、权限分支、数据转换、请求封装、页面关键交互）默认必须按以下顺序执行：
  1. 先写/更新相关测试（优先 co-located `*.test`）。
  2. 先跑受影响测试，确认失败可见到需求断言。
  3. 实现改动。
  4. 再跑受影响测试并确认通过。
- 仅文案、纯样式、一次性原型且无可观测行为边界的改动可讨论豁免，必须在交付说明里写“未测试原因 + 替代验证”。
- 任务结束时，交付说明必须列出“先测后改”的执行证据：受影响命令与测试结果摘要（或明确写清失败/缺失原因）。

### 提测前最小门禁（行为改动）

- 行为改动默认最小提测前门禁为：  
  1. `pnpm --filter <包名> exec tsc --noEmit`（或该子项目对应的类型命令）  
  2. `pnpm --filter <包名> check`（或等价静态检查）  
  3. 受影响受测集合（先做最小范围，必须先看到失败再改）  
  4. 关键行为改动再补一次 `test:cov`（有条件）或对应子项目 `test`/`test:cov` 最小门限。
- 子项目可以在自身 `AGENTS.md` 收紧覆盖范围（例如 web/life-trace 的 `test:cov` 最小范围要求）；根约束给出统一执行顺序，不替代子项目专属约束。

## 新增子项目 AGENTS 的复用模板（可直接套用）

当创建或重写任一子项目 `AGENTS.md` 时，建议保留以下四段（命令按实际脚本替换）：

- [模板源文件：`AGENTS_TEMPLATE.md`](./.agents/AGENTS_TEMPLATE.md)（建议先复制到新建的 `AGENTS.md`，再按项目实际命令替换占位符）。

- 1）`## AI Coding 约束（行为改动默认顺序）`
  1. 先补齐/更新相关测试。
  2. 先跑受影响测试并确认失败可见。
  3. 再改实现。
  4. 再跑受影响测试并确认通过。
  5. 仅文案/纯样式且无行为边界可豁免，但需在交付说明写“未测原因 + 替代验证”。

- 2）`## 本地 Preflight 约束（AI Coding 默认前置）`
  1. 子项目静态检查（如 `check`）。
  2. 子项目类型检查（如 `tsc --noEmit`/`go test` 的等价命令）。
  3. 行为相关受影响最小测试集合，要求先失败后改。

- 3）`## 校验要求` 中补充 `行为类高风险提测前最小门禁`
  - 统一要求：
    1. 静态检查通过。
    2. 类型/编译层检查通过。
    3. 受影响行为测试通过（或等价最小回归）。
  - 若无相应 `test` 脚本，改用最接近行为边界的替代验证并在交付说明标注。

- 4）如有跨层联动，在校验要求增加：`联动检查前端/服务端接口契约、配置同步`。
