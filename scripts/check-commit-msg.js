...
// Emoji + Conventional Commits 格式检查
// 支持两种格式：
//   1. emoji 前缀：✨ feat: 新功能 / 🐛 fix(scope): 修复
//   2. 纯文本：feat: 新功能 / fix(scope): 修复
const types =
  'feat|fix|docs|style|refactor|test|chore|perf|ci|build|revert|wip|experiment|assets|db|ui';
const emojiPattern = new RegExp(
  `^([\\p{Emoji_Presentation}\\u{FE0F}\\u{1F3FB}-\\u{1F3FF}\\u{E0020}-\\u{E007F}]\\s+)?(${types})(\\(.+\\))?: .{1,}`,
  'u',
);
// 允许 Git 自动生成的 merge commit message（如：Merge branch 'master' of ...）
const mergePattern = /^Merge (branch|remote-tracking branch) '.+'(?: of .+)?$/;

if (!emojiPattern.test(msg) && !mergePattern.test(msg)) {
  console.log('');
  console.log('❌ Commit message 格式错误！');
  console.log('');
  console.log('📝 正确格式：');
  console.log('   <emoji> <type>(<scope>): <subject>');
  console.log('   <emoji> <type>: <subject>');
  ...
  console.log('   refactor: 重构          ♻️');
  console.log('   test:     测试相关      ✅');
  console.log('   chore:    构建/工具     🔧');
  console.log('   perf:     性能优化      ⚡️');
  console.log('   ci:       CI/CD 相关    🚀');
  console.log('   build:    构建相关      🔧');
  console.log('   revert:   回退          🗑️');
  console.log('   wip:      进行中        🚧');
  console.log('');
  console.log('✅ 示例：');
  console.log('   ✨ feat: 添加用户认证');
  console.log('   🐛 fix(api): 修复登录超时');
  console.log('   📝 docs: 更新 README');
